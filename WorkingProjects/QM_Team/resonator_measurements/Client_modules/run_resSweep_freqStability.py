from mResSweep_freqStability import *
...
soc, soccfg = makeProxy('192.168.1.133')
print(soccfg)
...
# input['mixerCenter_f'] = 1462.9975  # center frequency for the mixer
# input['span_f'] = 0.015  # frequency span to sweep over. The same for all resonators

# input['res_f'] = [soc.reg2freq(5.9749286e3, gen_ch=6), 6.281788e3, 6.85059e3, 7.310013e3]  # center frequencies of the resonators
input['res_f'] = [soc.reg2freq(-20003, gen_ch=6)+6500, 6.281788e3, 6.85059e3, 7.310013e3]  # center frequencies of the resonators
# input['res_f'] = [i-0.84 for i in input['res_f']]



# define numbers of points
input['n_expts'] = 1001  # number of points along the frequency axis
input['n_rounds'] = 1  # number of times to sweep along the frequency axis
input['n_reps'] = 1122  # number of repetitions to take at each frequency point

# timing. Units are clock cycles, but converted from us
input['ring_up_time'] = soc.us2cycles(500)  # time waiting for the resonator to ring up at the start of each sweep
input['ring_between_time'] = soc.us2cycles(10)  # time to wait between adjacent frequency points
input['readout_length'] = soc.us2cycles(20)  # time to average over
