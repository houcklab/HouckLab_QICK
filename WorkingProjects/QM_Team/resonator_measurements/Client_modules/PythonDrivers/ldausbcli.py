...
class CLI_Vaunix_Attn:
    ...
    # Set Attenuation
    def set_attenuation(self, devid, attenuation, channel=''):
        try:
            if channel =='':
                ...
                self.vnx.fnLDA_InitDevice(devid)
                self.vnx.fnLDA_SetAttenuationHRQ(devid, int(attenuation)*20, channel)
                self.vnx.fnLDA_CloseDevice(devid)
        except:
    ...
    def get_channels_number(self, devid):
        try:
            return self.vnx.fnLDA_GetNumChannels(devid)
        except:
