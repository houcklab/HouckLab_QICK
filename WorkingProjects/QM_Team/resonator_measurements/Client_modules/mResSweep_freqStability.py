...
class ResSweep(ExperimentClass):
    ...
    def acquire(self, progress=False, debug=False):
        ...
        ampArrayFindFreq = [np.asarray([0. for ii in self.mixerArray_f]) for i in range(4)]
        fAtMin = [np.asarray([0. for ii in range(self.n_rounds)]) for i in range(4)]
        ...
        for roundInd in range(self.n_rounds):
            ...
            for IQInd in range(4):
                for freqInd, iq in enumerate(iqListRound):
                    ampArrayFindFreq[IQInd][freqInd] = abs(np.asarray(iq[0][IQInd]) + 1j*np.asarray(iq[1][IQInd]))
                    ampArrayTMP[IQInd][freqInd] += abs(np.asarray(iq[0][IQInd]) + 1j*np.asarray(iq[1][IQInd]))

            #find frequency with resonator center
            for IQInd in range(4):
                fAtMin[IQInd][roundInd] = self.resArray_f[IQInd][ampArrayFindFreq[IQInd].argmin()]
        ...
        ampArray_log = np.log10(ampArray)
    ...
    def display(self, data=None, fit=True, **kwargs):
        ...
        for i in range(4):
            axs[i].plot(data['fAtMin'][i],'.')
            axs[i].set_ylabel('Resonator center frequency [MHz]')
            axs[i].set_xlabel('Round #')
            axs[i].set_title(
                'f = {0:.0f} MHz'.format(self.res_f[i]))

        plt.savefig(self.iname + '_freqStability.png', bbox_inches='tight')
