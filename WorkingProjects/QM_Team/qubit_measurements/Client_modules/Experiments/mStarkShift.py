...
from WorkingProjects.QM_Team.qubit_measurements.Client_modules.Experiments.mSpecSliceFF import QubitSpecSliceFFProg
from WorkingProjects.QM_Team.qubit_measurements.Client_modules.Experiments.mTransmissionFF import CavitySpecFFProg
...
class StarkShift(ExperimentClass):
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, plotSave = True, figNum = 1):
        expt_cfg = {
            ...
            #spec parameters
            "qubit_freq_start": self.cfg["qubit_freq"] - self.cfg['SpecSpan']/2,  # [MHz] actual frequency is this number + "cavity_LO"
            "qubit_freq_stop": self.cfg["qubit_freq"] + self.cfg['SpecSpan']/2,  # [MHz] actual frequency is this number + "cavity_LO"
        ...
        for i in range(expt_cfg["trans_gain_num"]):
            ...
            avgamp0 = np.abs(sig)
            avgphase = np.unwrap(np.angle(sig, deg=True))
            avgI = data_I
            avgQ = data_Q
    ...
    def _acquireSpecData(self):
        ...
        expt_cfg = {
            ### spec parameters
            "qubit_freq_start": self.cfg["qubit_freq"] - self.cfg['SpecSpan']/2,
            "qubit_freq_stop": self.cfg["qubit_freq"] + self.cfg['SpecSpan']/2,
            "SpecNumPoints": self.cfg["SpecNumPoints"],  ### number of points
        }
        # self.cfg["reps"] = self.cfg["spec_reps"]
        # print("Reps = ", self.cfg["reps"])
        # print("Rounds = ", self.cfg["rounds"])
        ...
        # print(avgi.shape)
        data = {'config': self.cfg, 'data': {'x_pts': x_pts, 'avgi': avgi[0][0], 'avgq': avgq[0][0]}}
