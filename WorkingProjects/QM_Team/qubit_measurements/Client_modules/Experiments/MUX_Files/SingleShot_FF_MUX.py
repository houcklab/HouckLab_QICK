...
from WorkingProjects.QM_Team.qubit_measurements.Client_modules.Experiments.MUX_Files.MUXInitialize import *
from WorkingProjects.QM_Team.qubit_measurements.Client_modules.Experiments.MUX_Files.mSingleTone import SingleTone
...
#To do: check if this is correct. Maybe instead need * sum of the gains
gains = [Qubit_Parameters[str(Q_R)]['Readout']['Gain'] / 32767. * len(Qubit_Readout) for Q_R in Qubit_Readout]
...
Spec_relevant_params = {"qubit_gain":1000, "SpecSpan": 3, "SpecNumPoints": 41,
                        "reps": 25, 'rounds': 30,
                        'Gauss': True, "sigma": 0.4, "gain": 4000}
...
Amplitude_Rabi_params = {"qubit_freq": Qubit_Parameters[str(Qubit_Pulse[0])]['Qubit']['Frequency'],
                         "sigma": 0.4, "max_gain": 10000,
                         "reps": 25, 'rounds': 10,}

RunT1 = False
RunT2 = False
RunT2E = False
T1T2_params = {"T1_step": 50, "T1_expts": 100, "T1_reps": 20, "T1_rounds": 25 * 0 + 2,
...
if RunTransmissionSweep:
    ...
    CavitySpecFFMUX.save_config(Instance_trans)
...
if Run2ToneSpec:
    ...
    QubitSpecSliceFFMUX.save_config(Instance_specSlice)
...
if RunT1:
    ...
    T1MUX.display(iT1, dT1, plotDisp=plot_disp, figNum=2)
    T1MUX.save_data(iT1, dT1)
    T1MUX.save_config(iT1)
