...
class CavitySpecFFProg(AveragerProgram):
    def initialize(self):
        ...
        for iCh, ch in enumerate(cfg["ro_chs"]):  # configure the readout lengths and downconversion frequencies
            # self.declare_readout(ch=ch, length=self.us2cycles(cfg["readout_length"]), #, gen_ch = ch),
            #                      freq=cfg["pulse_freqs"][iCh], gen_ch=cfg["res_ch"])
            self.declare_readout(ch=ch, length=self.us2cycles(cfg["readout_length"]), #, gen_ch = ch),
                                 freq=cfg["pulse_freqs"][iCh] - 0.0000001, gen_ch=cfg["res_ch"])
            print(cfg["pulse_freqs"][iCh])
