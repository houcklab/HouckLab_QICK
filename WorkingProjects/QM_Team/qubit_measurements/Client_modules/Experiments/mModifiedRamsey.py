...
class ModifiedRamseyProgram(AveragerProgram):
    ...
    def initialize(self):
        ...
        #
        # The offset must exceed the largest |raw I| the ACCUMULATOR can
        # produce, not merely raw_threshold: a fixed constant is only correct
        # for one readout length. Both scale with the window -- raw_threshold =
        # readout_threshold * window_cycles, and |raw I| <= window_cycles * 2^15
        # -- so the offset is sized from the window in initialize() below
        # (int(ro_norm) << 15) rather than hardcoded. For reference, at the
        # deployed 15 us / 4608-cycle window the worst-case |raw I| is
        # 150,994,944: a 2^24 offset covers only 0.111x of that (shots below
        # -2^24 still wrap, i.e. the original bug survives), and while 2^28
        # does cover it, it silently stops doing so past a ~26.7 us readout.
        # 0 here; set for real in initialize() when active reset is on.
        self.cmp_offset = 0
