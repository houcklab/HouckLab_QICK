...
from WorkingProjects.QM_Team.qubit_measurements.Client_modules.Experiments.mConstantTone import ConstantTone_Experiment
...
outerFolder = "Z:\\t1Team\\Data\\2024-08-09_cooldown\\TATQ01-Si_01-AD\\7p0138\\"
...
UpdateConfig = {
    ...
    "trans_freq_start": 7219 - 0.5, #6735.456810631229 - 2,  # [MHz] actual frequency is this number + "cavity_LO" use span for +/- part
    "trans_freq_stop": 7219 + 0.5, #6735.456810631229 + 2,  # [MHz] actual frequency is this number + "cavity_LO"
    "TransNumPoints": 101,
...
UpdateConfig_transmission={
    ...
    "read_pulse_freq": 6932.6, #6735.456810631229, # #7194.787174129353, # [MHz] actual frequency is this number + "cavity_LO"
...
UpdateConfig_qubit = {
    ...
    "qubit_freq_start": 3417.4 - 1,
    "qubit_freq_stop": 3417.4 + 1,
...
# config["qubit_gain_step"] = 500
# config["qubit_gain_expts"] = 20
# config["AmpRabi_reps"] = 1000
...
UpdateConfig = {
    ...
    "read_pulse_gain": 1500,                # resonator drive power [DAC units]
    "read_pulse_freq": 7219.0,              # resonator frequency [MHz]
    ## qubit parameters ##
    "qubit_freq_start": 3954.25 - 10,           # first qubit freq point [MHz]
    "qubit_freq_stop": 3954.25 + 10,            # last qubit freq point [MHz]
    "RabiNumPoints": 15,                     # division of qubit freq range
    "qubit_pulse_style": "arb",             # arb: gaussian
    "sigma": 0.4,                           # 4 * sigma = pulse length [us]
    "flat_top_length": 3,                   # [us]
    "relax_delay": 3000,                    # time between experiments [us]
    ## amplitude rabi parameters ##
    "qubit_gain_start": 0,               # first qubit gain point [DAC]
    "qubit_gain_step": 1000,                # qubit gain step (max gain for RFSoC is 30000)
    "qubit_gain_expts": 14,                 # number of steps
    "AmpRabi_reps": 1000,                    # number of averages
}
...
for trial in range(trials):
    ...
    UpdateConfig = {
        ...
        "read_pulse_gain": 2000,       # resonator drive [DAC units]
        "read_pulse_freq": 7013.8,     # resonator frequency [MHz]
        ### Qubit Parameters ###
        "qubit_freq": 3405.6,         # qubit frequency [MHz]
        "qubit_gain": 6270,           # qubit drive [DAC units]
        "sigma": 0.2,                  # 4 * sigma = pulse length [us]
        "qubit_pulse_style": "arb",    # arb: gaussian
        "relax_delay": 4000,          # time between experiments [us]
        ### T1 Parameters ###
        "start": 0,                    # [us]
        "step": 100,                   # [us]
...
UpdateConfig = {
    ...
    "start": 0.00, ### us
    "step": 0.5, ### us
    "expts": 40, ### number of experiemnts 800
...
UpdateConfig = {
    ...
    ##### spec parameters for finding the qubit frequency
    "qubit_freq": 3924.81, #no need for detunning for echo
    "pi_qubit_gain": 14000,
    "pi2_qubit_gain": 5000,
    "sigma": 0.2,  ### units us, define a 20ns sigma
    "qubit_pulse_style": "arb", #### arb means gaussain here
...
UpdateConfig = {
    ...
    "read_pulse_freq": 6932.74, #7060.803322259137, # [MHz]
    ##### qubit spec parameters
    "qubit_pulse_style": "arb",
    "qubit_gain": 4100,
    # "qubit_length": 10,  ###us, this is used if pulse style is const
