...
#from WorkingProjects.Tantalum_fluxonium_escher.Client_modules.Experiments.mSpecSlice import LoopbackProgramSpecSlice  20250123 Parth says this is buggy
...
class TwoToneTrans(AveragerProgram):
    ...
    def initialize(self):
        ...
        self.set_pulse_registers(ch=cfg["res_ch"], style=cfg["read_pulse_style"], freq=freq, phase=0, gain=cfg["pop_pulse_gain"], #mode='periodic',
    ...
    def body(self):
        ...
        freq = self.freq2reg(self.cfg["read_pulse_freq"], gen_ch=self.cfg["res_ch"], ro_ch=self.cfg["ro_chs"][0])
        self.set_pulse_registers(ch=self.cfg["res_ch"], style=self.cfg["read_pulse_style"], freq=freq, phase=0,
                                 gain=self.cfg["pop_pulse_gain"],  # mode='periodic',
        ...
        self.set_pulse_registers(ch=self.cfg["res_ch"], style=self.cfg["read_pulse_style"], freq=freq, phase=0,
                                 gain=self.cfg["read_pulse_gain"],  # mode='periodic',
                                 length=self.us2cycles(self.cfg["read_length"], gen_ch=self.cfg["res_ch"]))

        self.sync_all(self.us2cycles(self.cfg["intermediate_delay"]))  # align channels and wait 50ns
...
class TwoToneTransVsFlux(ExperimentClass):
    """
    ...
    def _aquireTransData(self):
        ...
        for f in tqdm(fpts, position=0, disable=True):
            ...
            prog = TwoToneTrans(self.soccfg, self.cfg)
