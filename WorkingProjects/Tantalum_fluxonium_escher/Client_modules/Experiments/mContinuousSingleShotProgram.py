...
class LoopbackProgramContinuousSingleShot(AveragerProgram):
    ...
    def initialize(self):
        ...
        self.synci(100)
    def body(self):
        self.measure(pulse_ch=self.cfg["res_ch"], adcs=self.ro_chs, pins=[0], adc_trig_offset=self.cfg["adc_trig_offset"], wait=True, syncdelay=self.us2cycles(self.cfg["relax_delay"]))

class ContinuousSingleShotProgram(ExperimentClass):
    """
    ...
    def acquire(self, progress=False, debug=False):
        ...
        prog = LoopbackProgramContinuousSingleShot(self.soccfg, self.cfg)
        shots_i0,shots_q0 = prog.acquire(self.soc, load_pulses=True)

        print(np.shape(shots_i0))
        print(np.shape(shots_q0))
        i_g = shots_i0[0]
        q_g = shots_q0[0]
    ...
    def display(self, data=None, plotDisp = False, figNum = 1, save_fig = False, ran=None, **kwargs):
        if data is None:
            data = self.data
        ...
        # Display IQ traces
        # fig, axs = plt.subplots(nrows=2, ncols=1, figsize=(20, 10), num=1)
        # fig.suptitle('Continuous Single Shot Experiment')
        # axs[0].plot(i_g)
        # axs[0].set_title("I")
        # axs[1].plot(q_g)
        # axs[1].set_title("Q")
