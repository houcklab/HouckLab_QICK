...
import numpy as np
from WorkingProjects.Tantalum_fluxonium_escher.Client_modules.CoreLib.Experiment import ExperimentClass # Was WTF
...
class AmplitudeRabiFlux_PS(ExperimentClass):
    """
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, figNum = 1):
        ...
        self.data = {
            'config': self.cfg,
            'data': {'Z_mat': Z_mat,
                     'qubit_freqs': self.qubit_freqs, 'voltVec': self.voltVec,
                     }
        }

        ##### create arrays to store all the raw data
        i_0_arr = np.full((len(self.voltVec), len(self.qubit_freqs), int(self.cfg["shots"]) ), np.nan)
        q_0_arr = np.full((len(self.voltVec), len(self.qubit_freqs), int(self.cfg["shots"]) ), np.nan)
        i_1_arr = np.full((len(self.voltVec), len(self.qubit_freqs), int(self.cfg["shots"]) ), np.nan)
        q_1_arr = np.full((len(self.voltVec), len(self.qubit_freqs), int(self.cfg["shots"]) ), np.nan)
        ...
        for idx_yoko in range(len(self.voltVec)):
            ...
            for idx_freq in range(len(self.qubit_freqs)):
                ...
                i_0_arr[idx_yoko, idx_freq, :] = i_0
                q_0_arr[idx_yoko, idx_freq, :] = q_0
                i_1_arr[idx_yoko, idx_freq, :] = i_1
                q_1_arr[idx_yoko, idx_freq, :] = q_1

                ####################################################
                if idx_yoko == 0 and idx_freq == 0:
                    alpha_val = 0.5
                    colors = ['b', 'r', 'm', 'c', 'g']
                    ### store the data
                    i_raw = i_0
                ...
                for idx_cen in range(cen_num):
                    Z_mat[idx_cen][idx_yoko, idx_freq] = pops_arr[idx_cen, idx_cen]

                ####### plotting data
                if idx_freq == 0 and idx_yoko==0:  #### if first sweep add a colorbar
                    ...
                    for idx_cen in range(cen_num):
                        ...
                        axs[idx_cen].set_ylabel("yoko voltage (V)")
                ...
                if idx_freq == 0 and idx_yoko ==0:  ### during the first run create a time estimate for the data aqcuisition
