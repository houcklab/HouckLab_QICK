### Loop over TimeRabi with qubit frequency
...
class TimeRabi_Blob(ExperimentClass):
    """
    Sweep over qubit frequencies to drive a time Rabi chevron
    """
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, figNum = 1):
        ...
        expt_cfg = {
            ...
            ### rabi time parameters
            "qubit_len_start": self.cfg["qubit_len_start"],
            "qubit_len_step": self.cfg["qubit_len_step"],
            "qubit_len_expts": self.cfg["qubit_len_expts"], #### plus one to account for 0 index
        }
        ...
        self.qubit_lens = np.arange(0, expt_cfg["qubit_len_expts"]) * expt_cfg["qubit_len_step"] + expt_cfg["qubit_len_start"]
        ...
        self.data = {
            'config': self.cfg,
            'data': {'avgi_mat': Z_avgi, 'avgq_mat': Z_avgq,'qubit_freqs': self.qubit_freqs, 'qubit_lens': self.qubit_lens}
        }
