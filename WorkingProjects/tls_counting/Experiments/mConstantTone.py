...
import Pyro4
from WorkingProjects.tls_counting.Calib.initialize import BaseConfig
from WorkingProjects.tls_counting.CoreLib.Experiment import ExperimentClass
from WorkingProjects.tls_counting.CoreLib.socProxy import makeProxy
...
class ConstantTone(AveragerProgram):
    ...
    def initialize(self):
        freq = self.freq2reg(
            self.cfg["freq"], gen_ch=self.cfg["channel"], ro_ch=self.cfg["ro_chs"][0]
...
if __name__ == "__main__":
    ...
    outerFolder = "Z:\\FluxTeam\\Data\\rfsoc_test\\"
    ...
    try:
        ConstantTone_Instance = ConstantTone_Experiment(
            path="test_constant_tone",
            outerFolder=outerFolder,
            cfg=config,
            soc=soc,
            soccfg=soccfg,
