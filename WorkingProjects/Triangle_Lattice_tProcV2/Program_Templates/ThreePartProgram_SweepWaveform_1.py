...
class ThreePartProgram_SweepOneFF(FFAveragerProgramV2):
    def _initialize(self, cfg):
        ...
        for channel in self.FFChannels:
            for i in range(1,17):
                wname = f"FFExpt_{i}_{channel}_w0"
                ...
                self.write_reg(dst='w_length', src='cycle_counter')
        ...
        for channel in self.FFChannels:
            for i in range(1,17):
                wname = f"FFExpt_{i}_{channel}_w0"
                # print(self.list_pulse_waveforms(f"FFExpt_{i}_{channel}"))
                IncrementLength.read_wmem(name=wname)
                IncrementLength.inc_reg(dst='w_length', src= + 1)
                IncrementLength.write_wmem(name=wname)
    ...
    def _body(self, cfg):
        ...
        # FF.FFPlayWaveforms(self, f"FFExpt_1")
        # 2: FFExpt
        ...
        for i in range(1,17):
            self.label(f"l{i}")
            FF.FFPlayWaveforms(self, f"FFExpt_{i}", t_start=0)
            self.jump("finish")
