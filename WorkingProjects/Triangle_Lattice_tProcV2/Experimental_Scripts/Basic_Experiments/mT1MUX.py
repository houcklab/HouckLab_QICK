...
class T1MUX(ExperimentClass):
    ...
    def _fit_t1(self, x_pts, Contrast):
        ...
        on success; on failure leaves them unset and prints a message. Run
        from ``acquire`` so it is available regardless of whether the caller
        invokes ``display``.
        """
        self.T1 = None
        self.t1_A = None
        ...
        try:
            ...
            return
        # Reject obviously bogus fits (negative or wildly long T1).
        max_t = float(x_pts[-1])
    ...
    def acquire(self, progress=False):
        ...
        # Fit T1 here so it is available to on_apply / display / auto-cal flows.
    ...
    def display(self, data=None, plotDisp = False, figNum = 1, block=True, ax=None, **kwargs):
        ...
        if T1 is not None and getattr(self, "t1_fit_curve", None) is not None:
            sign = np.sign(self.t1_A)  # force plot to go down
