...
class RamseyFFCalRProg(RAveragerProgram):
    def initialize(self):
        ...
        self.PULSE_NUM_REG = 1 #register for counting pulse number
        self.CONST_PERIOD_REG = 2 # wraparound period, contains 16
        self.STORE_ADDR_REG = 3 # stores value of r_addr because we play multiple pulses
        self.STORE_MODE_REG = 4

        for rp, r_addr, r_mode in zip(self.rps, self.r_addrs, self.r_modes):
            # !--- here I set a useful counter for iterating through pulses ---!
            self.regwi(rp, self.PULSE_NUM_REG, 0)  # counting pulse number (0 to 15)
            self.regwi(rp, self.CONST_PERIOD_REG, 16)  # wraparound period
            ...
            self.regwi(rp, self.STORE_ADDR_REG, 0)  # stores value of r_addr because we play multiple pulses

            # !--- here I set the pulse length with registers to be the shortest possible ---!
            self.mathi(rp, r_mode, r_mode, '-', self.cfg["FFlength"] // 16 - 1)  # shortest len
            self.mathi(rp, self.STORE_MODE_REG, r_mode, '+', 0)  # stores value of res_r_mode because we play multiple pulses
    ...
    def body(self):
        ...
        for FFChannel, rp, r_addr, r_mode in zip(self.FFChannels, self.rps, self.r_addrs, self.r_modes):
            ...
            self.mathi(rp, r_addr, self.STORE_ADDR_REG, '+', 0)  # restore value of r_addr
            self.mathi(rp, r_mode, self.STORE_MODE_REG, '+', 0)  # restore value of r_mode
        ...
        for FFChannel, rp, r_g, r_addr, r_mode in zip(self.FFChannels, self.rps, self.r_gains, self.r_addrs,
                                                          self.r_modes):
            self.set_pulse_registers(ch=FFChannel, freq=0, phase=0, gain=self.soccfg['gens'][FFChannel]['maxv'],
                                     style='arb', waveform="1", outsel="input")
            self.mathi(rp, r_addr, self.STORE_ADDR_REG, '+', 0)  # restore value of r_addr
    ...
    def update(self):
        ...
        self.memwi(self.rps[3], self.PULSE_NUM_REG, 123)
        self.memwi(self.rps[3], self.CONST_PERIOD_REG, 124)
        self.memwi(self.rps[3], self.STORE_ADDR_REG, 126)  # skip address 125
        self.memwi(self.rps[3], self.STORE_MODE_REG, 127)
        ...
        for i, rp in enumerate(set(self.rps)):  # increment once per page! some channels use the same page
            ...
            self.mathi(rp, self.PULSE_NUM_REG, self.PULSE_NUM_REG, '+', self.cfg['step'])  # FIXME: assumes step == 1
            self.mathi(rp, self.STORE_ADDR_REG, self.STORE_ADDR_REG, '+', self.cfg['step'] * (self.cfg['FFlength'] // 16 + 2))

            # if counter >= 16, reset to 0 and add one to length counter
            self.condj(rp, self.PULSE_NUM_REG, '<', self.CONST_PERIOD_REG, 'NO_WRAP'+str(i))
            # self.condj(rp, 3, '<', 2, 'NO_WRAP'+str(i))
            self.regwi(rp, self.PULSE_NUM_REG, 0)
            self.regwi(rp, self.STORE_ADDR_REG, 0)  # reset pulse address to beginning
            self.mathi(rp, self.STORE_MODE_REG, self.STORE_MODE_REG, '+', 1)  # increase length by 1 clock cycle
