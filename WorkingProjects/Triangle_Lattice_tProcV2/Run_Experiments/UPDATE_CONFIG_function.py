def update_config(**kwargs):
    ...
    Qubit_Parameters = kwargs["Qubit_Parameters"]
    Qubit_Readout = kwargs["Qubit_Readout"]
    Qubit_Pulse = kwargs["Qubit_Pulse"]



    FF_Qubits = kwargs["FF_Qubits"]

    Init_FF = kwargs["Init_FF"]

    BaseConfig = kwargs["BaseConfig"]
    ...
    FFExpt = [kwargs[f'FF_gain{i + 1}_expt'] for i in range(len(FFReadouts))]
    FF_BS = [kwargs[f'FF_gain{i + 1}_BS'] for i in range(len(FFReadouts))]
