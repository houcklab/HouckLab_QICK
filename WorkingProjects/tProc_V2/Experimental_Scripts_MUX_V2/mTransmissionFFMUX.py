...
class CavitySpecFFProg(AveragerProgramV2):
    def _initialize(self, cfg):
        print(cfg["mixer_freq"])
        self.declare_gen(ch=cfg['res_ch'], ro_ch=cfg['ro_chs'][0], nqz=cfg["nqz"],
                         mixer_freq=cfg["mixer_freq"],
                         mux_freqs=cfg['swept_pulse_freqs'],
                         mux_gains=cfg['cavity_gains'])
        for ch, f in zip(cfg['ro_chs'], cfg['swept_pulse_freqs']):
            self.declare_readout(ch=ch, length=cfg['readout_length'], freq=f, gen_ch=cfg['res_ch'])

        self.add_pulse(ch=cfg['res_ch'], name="cavity_drive",
                       style="const",
                       length=cfg["length"],
                       mask=list(range(len(cfg["ro_chs"])))
                       )
...
class CavitySpecFFMUX(ExperimentClass):
    ...
    def acquire(self, progress=False):
        mixer_fpts = np.linspace(self.cfg["mixer_freq"] - self.cfg["TransSpan"],
                           self.cfg["mixer_freq"] + self.cfg["TransSpan"],
                           self.cfg["TransNumPoints"])
        pulse_fpts = [np.linspace(pulse_freq - self.cfg["TransSpan"],
                           pulse_freq + self.cfg["TransSpan"],
                           self.cfg["TransNumPoints"]) for pulse_freq in self.cfg["cavity_freqs"]]
        results = []
        start = time.time()
        for mf, *pfs in zip(mixer_fpts, *pulse_fpts):  # tqdm(fpts, position=0, disable=True):
            self.cfg["mixer_freq"] = mf
            self.cfg["swept_pulse_freqs"] = pfs
        ...
        print(results)
        data = {'config': self.cfg, 'data': {'results': results, 'fpts': pulse_fpts}}
        self.data = data
        ...
        self.peakFreq_min = data['data']['fpts'][0][peak_loc]
        peak_loc = np.argmax(avgamp0)
        self.peakFreq_max = data['data']['fpts'][0][peak_loc]
    ...
    def display(self, data=None, plotDisp = False, figNum = 1, **kwargs):
        ...
        for i in range(len(data['data']['results'][0])):
            ...
            x_pts = (data['data']['fpts'][i] + self.cfg["cavity_LO"] / 1e6) / 1e3  #### put into units of frequency GHz
