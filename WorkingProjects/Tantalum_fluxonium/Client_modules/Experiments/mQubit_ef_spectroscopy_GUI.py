...
class Qubit_ef_spectroscopy_GUI(ExperimentClass):
    """
    Basic AmplitudeRabi
    """
    ...
    def display(self, fig, data=None, plotDisp = False, figNum = 1, **kwargs):
        if data is None:
            data = self.data
        ...
        axs.plot(x_pts, avgsig, 'o-', label="Magnitude")
        axs.set_ylabel("a.u.")
        axs.set_xlabel("e-f spec frequency (MHz)")
        axs.legend()
