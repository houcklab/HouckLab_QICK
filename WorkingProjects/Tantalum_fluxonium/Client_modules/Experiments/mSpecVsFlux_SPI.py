...
class SpecVsFlux_SPI(ExperimentClass):
    """
    Spec experiment that finds the qubit spectrum as a function of flux, specifically it uses a SPI to sweep
    Notes;
        - this is set up such that it plots out the rows of data as it sweeps through SPI
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, plotSave = True, figNum = 1):
        expt_cfg = {
            ...
            "SPIVoltageStart": self.cfg["SPIVoltageStart"],
            "SPIVoltageStop": self.cfg["SPIVoltageStop"],
            "SPIVoltageNumPoints": self.cfg["SPIVoltageNumPoints"],
        ...
        voltVec = np.linspace(expt_cfg["SPIVoltageStart"],expt_cfg["SPIVoltageStop"], expt_cfg["SPIVoltageNumPoints"])

        D5a.set_voltage(0, expt_cfg["SPIVoltageStart"])
        ...
        Y = voltVec
        Y_step = Y[1] - Y[0]
        Z_trans = np.full((expt_cfg["SPIVoltageNumPoints"], expt_cfg["TransNumPoints"]), np.nan)
        Z_specamp = np.full((expt_cfg["SPIVoltageNumPoints"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_specphase = np.full((expt_cfg["SPIVoltageNumPoints"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_specI = np.full((expt_cfg["SPIVoltageNumPoints"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_specQ = np.full((expt_cfg["SPIVoltageNumPoints"], expt_cfg["SpecNumPoints"]), np.nan)

        ### create an initial data dictionary that will be filled with data as it is taken during sweeps
        self.trans_Imat = np.zeros((expt_cfg["SPIVoltageNumPoints"], expt_cfg["TransNumPoints"]))
        self.trans_Qmat = np.zeros((expt_cfg["SPIVoltageNumPoints"], expt_cfg["TransNumPoints"]))
        self.spec_Imat = np.zeros((expt_cfg["SPIVoltageNumPoints"], expt_cfg["SpecNumPoints"]))
        self.spec_Qmat = np.zeros((expt_cfg["SPIVoltageNumPoints"], expt_cfg["SpecNumPoints"]))
        self.data= {
            'config': self.cfg,
        ...
        #### loop over the SPI vector
        for i in range(expt_cfg["SPIVoltageNumPoints"]):
            ### set the SPI voltage for the specific run
            D5a.set_voltage(0, voltVec[i])
            ...
            axs['b'].set_ylabel("SPI voltage (V)")
            ...
            axs['a'].set_ylabel("SPI voltage (V)")
            ...
            axs['c'].set_ylabel("SPI voltage (V)")
            ...
            axs['d'].set_ylabel("SPI voltage (V)")
            ...
            axs['e'].set_ylabel("SPI voltage (V)")
            ...
            if i ==0: ### during the first run create a time estimate for the data aqcuisition
                ...
                timeEst = t_delta*expt_cfg["SPIVoltageNumPoints"] ### estimate for full scan
