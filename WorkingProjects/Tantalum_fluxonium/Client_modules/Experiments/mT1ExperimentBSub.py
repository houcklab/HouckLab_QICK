...
class LoopbackProgramT1Experiment2(RAveragerProgram):
    ...
    def update(self):
        self.mathi(self.q_rp, self.r_wait, self.r_wait, '+',
                   self.us2cycles(self.cfg["step"]))  # update gain of the Gaussian pi pulse
...
class T1Experiment2(ExperimentClass):
    ...
    def acquire(self, progress=False, debug=False):
        ...
        prog_bkg = LoopbackProgramT1Experiment2(self.soccfg, self.cfg)
        x_pts_bkg, avgi_bkg, avgq_bkg = prog_bkg.acquire(self.soc, threshold=None, angle=None, load_pulses=True,
                                             readouts_per_experiment=1, save_experiments=None,
                                             start_src="internal", progress=False, debug=False)

        data = {'config': self.cfg, 'data': {'times': x_pts, 'avgi': avgi- avgi_bkg, 'avgq': avgq- avgq_bkg}}
        self.data = data
