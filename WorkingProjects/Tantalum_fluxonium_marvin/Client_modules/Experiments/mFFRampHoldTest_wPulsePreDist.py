...
class FFRampHoldTest(NDAveragerProgram):
    ...
    def build_ff_pulse(self, dt_pulsedef = 0.25, dt_pulseplay = 5):
        ...
        The pulse is constructed using small-time steps where the amplitude is constant.
        ...
        seg1_end = int((self.cfg["qubit_length"] + self.cfg["pre_meas_delay"] + self.cfg["read_length"] + self.cfg['relax_delay_1'])/dt_pulsedef) + 1
        seg2_beg = int((self.cfg["qubit_length"] + self.cfg["pre_meas_delay"] + self.cfg["read_length"] + self.cfg['relax_delay_1'] + self.cfg["ff_ramp_length"])/dt_pulsedef) + 1
        seg2_end = int((self.cfg["qubit_length"] + self.cfg["pre_meas_delay"] + self.cfg["read_length"] + self.cfg['relax_delay_1'] + self.cfg["ff_ramp_length"] + self.cfg["ff_hold"])/dt_pulsedef) + 1
        seg3_beg = int((self.cfg["qubit_length"] + self.cfg["pre_meas_delay"] + self.cfg["read_length"] + self.cfg['relax_delay_1'] + self.cfg["ff_ramp_length"] + self.cfg["ff_hold"]+ self.cfg["ff_ramp_length"])/dt_pulsedef) + 1
        ...
        # seg3_play = np.append(seg3_play, seg3_)
        ...
        if self.cfg.get('zeroing_pulse', False) and self.cfg.get("pulse_pre_dist", False):
            x_pulse = waveform
            T_opt, amps, edges = model.design_four_tail_zeroing_with_amax(x_pulse,
                                                                          a_max=self.cfg.get("zeroing_a_max", 30000))
            self.t_opt = T_opt
