...
class LoopbackProgramPumpProbeSingleShot(RAveragerProgram):
    ...
    def initialize(self):
        ...
        #### Sweep over the amplitude of the pump readout pulse
        cfg["start"] = cfg["pump_start"]
        cfg["step"] = cfg["pump_step"]
        cfg["reps"] = cfg["shots"]
        cfg["expts"] = cfg["num_pumps"]
        ...
        if cfg["qubit_pulse_style"] == "arb":
            print("Sending a Gaussian Pulse")
            self.add_gauss(ch=cfg["qubit_ch"], name="qubit",
                           sigma=self.us2cycles(self.cfg["sigma"], gen_ch=qubit_ch),
                           length=self.us2cycles(self.cfg["sigma"], gen_ch=qubit_ch) * 4)
            ...
            self.set_pulse_registers(ch=cfg["qubit_ch"], style=cfg["qubit_pulse_style"], freq=qubit_freq,
                                     ...
                                     waveform="qubit",  length=self.us2cycles(self.cfg["flat_top_length"], gen_ch=cfg["qubit_ch"]))
...
class PumpProbeSingleShot(ExperimentClass):
    """
    Pump probe single shot which sweeps the amplitude of an intermediate populating (pump) pulse at the readout frequency
    """
    ...
    def acquire(self, progress=False, debug=False):
        ...
        prog = LoopbackProgramPumpProbeSingleShot(self.soccfg, self.cfg)
