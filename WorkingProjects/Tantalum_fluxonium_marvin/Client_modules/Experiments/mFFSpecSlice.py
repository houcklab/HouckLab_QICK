...
class FFSpecSlice(NDAveragerProgram):
    ...
    def body(self):
        ...
        if self.cfg["ff_pulse_style"] == "const":
            ...
            qubit_spec_delay_cycles = self.us2cycles(self.cfg["qubit_spec_delay"], gen_ch = self.cfg["qubit_ch"])
            adc_trig_offset_cycles = self.us2cycles(self.cfg["adc_trig_offset"])
            ...
            self.sync_all(self.us2cycles(
                self.cfg['pre_meas_delay']))  # In case the channels are somewhat misaligned, wait a few tens of ns
...
class FFSpecSlice_Experiment(ExperimentClass):
    ...
    def acquire(self, progress=False, debug=False):
        ...
        self.qubit_peak_freq = self.qubit_freqs[np.argmin(mag)]
