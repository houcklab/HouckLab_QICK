...
from WorkingProjects.Tantalum_fluxonium_marvin.Client_modules.Experiments.mAmplitudeRabi import LoopbackProgramAmplitudeRabi
...
class RabiChevronEnergy(ExperimentClass):
    """
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, figNum = 1):
        ...
        print("Maximum length of pulse const and flat-top is 150us. Maximum sigma of arb and flat-top is 2us ")
        ...
        # Adjust the config to match with specslice
        self.cfg['SpecNumPoints'] = self.cfg['RabiNumPoints']
        ...
        # Scaling pulse length with qubit amps
        if self.cfg['qubit_pulse_style'] == 'flat_top':
            self.sigma = self.cfg['sigma']*self.qubit_amps[0]/self.qubit_amps
            self.flat_top_length = self.cfg['flat_top_length']*self.qubit_amps[0]/self.qubit_amps

            if debug :
                print("Sigma array is ", self.sigma)
                print("Flat top length array is", self.flat_top_length)

        elif self.cfg['qubit_pulse_style'] == 'const':
            self.qubit_length = self.cfg['qubit_length']*self.qubit_amps[0]/self.qubit_amps

            if debug:
                print("Qubit length array is ", self.qubit_length)

        elif self.cfg["qubit_pulse_style"] == 'arb':
            self.sigma = self.cfg['sigma']*self.qubit_amps[0]/self.qubit_amps

            if debug:
                print("Sigma array is ", self.sigma)

        # Setting the config for experiment
        self.cfg["reps"] = self.cfg["AmpRabi_reps"]
        self.cfg["start"] = expt_cfg["qubit_freq_start"]
        self.cfg["step"] = (expt_cfg["qubit_freq_stop"] - expt_cfg["qubit_freq_start"]) / expt_cfg["RabiNumPoints"]
        self.cfg["expts"] = expt_cfg["RabiNumPoints"]
        ...
        ### start the loop over qubit amplitudes
        for idx in range(len(Y)):
            # set new qubit gain and length and aquire data
            self.cfg["qubit_gain"] = self.qubit_amps[idx]
            if debug:
                print("Qubit Gain = " , self.cfg["qubit_gain"])
            if self.cfg['qubit_pulse_style'] == 'flat_top':
                self.cfg['sigma']  = self.sigma[idx]
                self.cfg['flat_top_length'] =  self.flat_top_length[idx]
                if debug:
                    print("Sigma = ", self.cfg['sigma'])
                    print("Flat_top_length = ", self.cfg['flat_top_length'])
            elif self.cfg['qubit_pulse_style'] == 'const':
                self.cfg['qubit_length'] = self.qubit_amps[idx]
                if debug:
                    print("Qubit Length = ", self.cfg['qubit_length'])
            elif self.cfg["qubit_pulse_style"] == 'arb':
                self.cfg['sigma'] = self.qubit_amps[idx]
                if debug:
                    print("Sigma = ", self.cfg['sigma'])
