...
class FFRampHoldPopTest(NDAveragerProgram):
    ...
    def body(self):
        ...
        # Adding a population oulse here to at the new flux values
        self.set_pulse_registers(ch=self.cfg["res_ch"], style=self.cfg["read_pulse_style"],
        ...
        # Play the population pulse
        self.pulse(ch = self.cfg["res_ch"], t = 'auto')
        self.sync_all(self.us2cycles(0.5))  # Wait a few ns to align channels
        ...
        # Resetting back to the normal readout pulse, after changing it to population pulse
        self.set_pulse_registers(ch=self.cfg["res_ch"], style=self.cfg["read_pulse_style"],
                                 freq=self.freq2reg(self.cfg["read_pulse_freq"], gen_ch=self.cfg["res_ch"],
                                                    ro_ch=self.cfg["ro_chs"][0]), phase=0,
                                 gain=self.cfg["read_pulse_gain"],
