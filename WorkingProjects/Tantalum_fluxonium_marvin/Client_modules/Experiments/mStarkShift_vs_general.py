...
class LoopbackProgramStarkSlice(RAveragerProgram):
    ...
    def body(self):
        ...
        self.set_pulse_registers(ch=self.cfg["res_ch"], style=self.cfg["read_pulse_style"], freq=self.f_res, phase=0,
                                 ...
                                 length=self.us2cycles(self.cfg["cavity_pulse_length"], gen_ch=self.cfg["res_ch"]))
...
class StarkShift(ExperimentClass):
    ...
    def acquire(self, progress=False, debug=False, plotDisp=True, plotSave=True, figNum=1):
        ...
        self.spec_Imat = np.zeros((expt_cfg["param_num"], expt_cfg["SpecNumPoints"]))
        self.spec_Qmat = np.zeros((expt_cfg["param_num"], expt_cfg["SpecNumPoints"]))
        self.data = {
            'config': self.cfg,
            'data': {'gain_fpts': paramVec,
        ...
        Z_specamp = np.full((expt_cfg["param_num"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_specphase = np.full((expt_cfg["param_num"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_specI = np.full((expt_cfg["param_num"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_specQ = np.full((expt_cfg["param_num"], expt_cfg["SpecNumPoints"]), np.nan)
        ...
        for i in range(expt_cfg["param_num"]):
            ...
            if self.cfg["calibrate_cav"]:
                ...
                transm_exp = Transmission_Enhance(path="TransmisionEnhanced", cfg=self.cfg, soc=self.soc,
                                                  soccfg=self.soccfg,
