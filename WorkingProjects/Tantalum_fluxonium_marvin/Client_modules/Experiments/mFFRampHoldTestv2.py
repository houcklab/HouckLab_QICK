...
class FFRampHoldTest(NDAveragerProgram):
    ...
    def initialize(self):
        ...
        if self.cfg["ff_ramp_style"] == "linear":
            ...
            if 'negative_pulse' in self.cfg.keys():
                if self.cfg['negative_pulse']:
                    if 'multiplier' not in self.cfg.keys():
                        self.cfg['multiplier'] = 1
                        print(f"Negative pulse multiplier not specified, setting to {self.cfg['multiplier']}")
                    if np.abs(self.cfg['ff_ramp_stop']*self.cfg['multiplier']) > self.soccfg['gens'][0]['maxv']:
                        raise ValueError("cfg[\"ff_ramp_stop\"]*cfg[\"multiplier\"] exceeds max value in magnitude.")
                    temp = self.cfg['ff_ramp_stop']
                    self.cfg['ff_ramp_stop'] = self.cfg['ff_ramp_stop']*self.cfg['multiplier']
                    PulseFunctions.create_ff_ramp(self, reversed=False, name = "ramp_neg")
                    PulseFunctions.create_ff_ramp(self, reversed=True, name = "ramp_neg_reversed")
                    self.cfg['ff_ramp_stop'] = temp # set it back for normal ramps
        else:
        ...
        if 'neg_init_fact' not in self.cfg.keys():
            self.cfg['neg_init_fact'] = 0.5
            print(f"Negative pulse initial factor not specified, setting to {self.cfg['neg_init_fact']}")
        else:
            print(f"Negative pulse initial factor = {self.cfg['neg_init_fact']}")
            if self.cfg['neg_init_fact'] > 1 or self.cfg['neg_init_fact'] < 0:
                print("neg_init_fact should be between 0 and 1. Setting to 0.5")
                self.cfg['neg_init_fact'] = 0.5
    ...
    def body(self):
        ...
        if self.cfg['negative_pulse'] :
            self.sync_all(self.us2cycles(1))
            if self.cfg["ff_hold"]*self.cfg["neg_init_fact"] > 0:
                self.set_pulse_registers(ch=self.cfg["ff_ch"], freq=0, style='arb', phase=0, stdysel='last',
                                         gain=-self.soccfg['gens'][0]['maxv'], waveform="ramp_neg", outsel="input")
                self.pulse(ch=self.cfg["ff_ch"], t='auto')
                self.sync_all(self.us2cycles(self.cfg["ff_hold"]*self.cfg["neg_init_fact"]))
                self.set_pulse_registers(ch=self.cfg["ff_ch"], freq=0, style='arb', phase=0,
                                         gain=-self.soccfg['gens'][0]['maxv'], waveform="ramp_neg_reversed", outsel="input")
                self.pulse(ch=self.cfg["ff_ch"], t='auto')
        ...
        if self.cfg['negative_pulse'] :
            self.sync_all(self.us2cycles(1))
            if self.cfg["ff_hold"]*(1 - self.cfg["neg_init_fact"]) > 0:
                self.set_pulse_registers(ch=self.cfg["ff_ch"], freq=0, style='arb', phase=0, stdysel='last',
                                         gain=-self.soccfg['gens'][0]['maxv'], waveform="ramp_neg", outsel="input")
                self.pulse(ch=self.cfg["ff_ch"], t='auto')
                self.sync_all(self.us2cycles(self.cfg["ff_hold"]*(1 - self.cfg["neg_init_fact"])))
                self.set_pulse_registers(ch=self.cfg["ff_ch"], freq=0, style='arb', phase=0,
                                         gain=-self.soccfg['gens'][0]['maxv'], waveform="ramp_neg_reversed", outsel="input")
                self.pulse(ch=self.cfg["ff_ch"], t='auto')
            self.sync_all(self.us2cycles(0.005))
