...
#from WorkingProjects.Tantalum_fluxonium_marvin.Client_modules.Experiments.mSpecSlice import LoopbackProgramSpecSlice
...
from WorkingProjects.Tantalum_fluxonium_marvin.Client_modules.Calib.initialize import yoko1
...
class SpecOnRepeat(ExperimentClass):
    """
    Experiment which performs repeated single and two-tone spec at a given flux point; test for flux drift
    Notes;
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, plotSave = True, figNum = 2, subtract_avg=True):
        expt_cfg = {
            ### define the yoko parameters
            "yokoVoltageStart": self.cfg["yokoVoltage"],
            "yokoVoltageStop": self.cfg["yokoVoltage"],
            "NumPoints": self.cfg["NumPoints"],
        ...
        voltVec = np.linspace(expt_cfg["yokoVoltageStart"],expt_cfg["yokoVoltageStop"], expt_cfg["NumPoints"])
        ...
        # fig_trans, axs_trans = plt.subplots(nrows=2,ncols=2, figsize=(8,10), num=1)
        # fig_spec, axs_spec = plt.subplots(nrows=2, ncols=2, figsize=(8, 10), num=2)
        ...
        Z_trans = np.full((expt_cfg["NumPoints"], expt_cfg["TransNumPoints"]), np.nan)
        Z_transphase = np.full((expt_cfg["NumPoints"], expt_cfg["TransNumPoints"]), np.nan)
        Z_transI = np.full((expt_cfg["NumPoints"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_transQ = np.full((expt_cfg["NumPoints"], expt_cfg["SpecNumPoints"]), np.nan)
        ...
        Y = np.linspace(0, expt_cfg["NumPoints"], expt_cfg["NumPoints"]+1)
        Y_step = Y[1] - Y[0]

        Z_specamp = np.full((expt_cfg["NumPoints"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_specphase = np.full((expt_cfg["NumPoints"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_specI = np.full((expt_cfg["NumPoints"], expt_cfg["SpecNumPoints"]), np.nan)
        Z_specQ = np.full((expt_cfg["NumPoints"], expt_cfg["SpecNumPoints"]), np.nan)

        ### create an initial data dictionary that will be filled with data as it is taken during sweeps
        self.trans_Imat = np.zeros((expt_cfg["NumPoints"], expt_cfg["TransNumPoints"]))
        self.trans_Qmat = np.zeros((expt_cfg["NumPoints"], expt_cfg["TransNumPoints"]))
        self.spec_Imat = np.zeros((expt_cfg["NumPoints"], expt_cfg["SpecNumPoints"]))
        self.spec_Qmat = np.zeros((expt_cfg["NumPoints"], expt_cfg["SpecNumPoints"]))
        self.data = {
            'config': self.cfg,
        ...
        yoko1.SetVoltage(voltVec[0])
        opt_pts = []
        y_pts = []

        #### Repeat over number of points
        for i in range(expt_cfg["NumPoints"]):
            y_pts.append(i)
            ...
            if i == 0:  #### if first sweep add a colorbar
                ...
                # ax_plot_0_dots, = axs['b'].plot(self.trans_fpts[np.argmin(avgamp0)]/1e3, i, 'ro')
            else:
            ...
            axs['a'].set_ylabel("Index")
            axs['a'].set_xlabel("Spec Frequency (GHz)")
            axs['a'].set_title("Qubit Spec on Repeat : Amplitude")
            ...
            if i == 0:  ### during the first run create a time estimate for the data aqcuisition
                ...
                timeEst = t_delta * expt_cfg["NumPoints"]  ### estimate for full scan
