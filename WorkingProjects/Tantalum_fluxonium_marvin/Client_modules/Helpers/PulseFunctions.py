...
class SimpleTwoTailDistortion:
    ...
    def _two_tail_amps_for_T(self,
        ...
        """
        Given:
            S   : array shape (2,) with S_i = ∫_0^{T1} e^{-(T - t)/τ_i} x_main(t) dt
            taus: array shape (2,) of tail time constants [τ1, τ2]
            T1  : time when main predistorted pulse ends
            T   : candidate final time (T > T1)

        Returns:
            amps  : array [a1, a2] for 2 cancelling square pulses
            edges : array of segment edges [T1, T2, T3=T]
        """
        taus = np.asarray(taus, float).ravel()
        assert taus.shape == (2,)
        assert T > T1

        # 2 equal segments between T1 and T
        edges = np.linspace(T1, T, 3)  # T1=edges[0], edges[2]=T

        # Build 2x2 M(T) with backward weighting
    ...
    def design_two_tail_zeroing_with_amax(self,
        ...
        Returns:
            ...
            amps  : amplitudes [a1, a2] of the cancelling pulses
            edges : segment edges [T1, T2, T3=T_opt]
        """
        x_pred_main = np.asarray(x_pred_main, float).ravel()
        taus = np.array([self.params.tau1, self.params.tau2], dtype=float)
        assert taus.shape == (2,)
        dt = self.dt
        ...
        if len(x_pred_main) < 2:
            raise ValueError("x_pred_main must have at least 2 samples for integration.")

        # --- 1) Precompute S_base_i = ∫_0^{T1} e^{-(T1 - t)/τ_i} x(t) dt
        t_main = np.arange(len(x_pred_main)) * dt
        S_base = np.zeros(2, dtype=float)
        for i in range(2):
            tau_i = taus[i]
        ...
        def amps_and_max(T: float) -> tuple[np.ndarray, float, np.ndarray]:
            ...
            S_T = S_base * decay
            amps, edges = self._two_tail_amps_for_T(S_T, taus, T1, T)
        ...
        while True:
            ...
            if T_high > T_limit:
                raise RuntimeError(
                    "Could not satisfy amplitude constraint within reasonable T. "
                    "Increase max_expand_factor."
                )
