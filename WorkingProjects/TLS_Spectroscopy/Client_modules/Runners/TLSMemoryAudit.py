...
def target_table(p=P, flux_fit_params=None, base_config=BaseConfig):
    ...
    search_min = float(p.get("dc_search_min", 0.0))
    ...
    if not np.isfinite([search_min, search_max]).all() or search_max <= search_min:
        raise ValueError("dc search bounds must be finite with max greater than min")
    f_edges = fx.estimate_fit_frequency_ghz_array(
        params, np.asarray([search_min, search_max]))
    f_min, f_max = float(np.min(f_edges)), float(np.max(f_edges))
    park_coordinate = float(base_config.get("ff_park_gain", 0.0) or 0.0)
    fit_park = fx.estimate_fit_frequency_ghz(params, park_coordinate)
    ...
    for role, offset in (("minus", -detuning), ("tls", 0.0), ("plus", detuning)):
        ...
        dc = float(fx.frequency_to_local_flux_branch(
            params, np.asarray([requested]), search_min, search_max)[0])
...
def main():
    with tee_log.tee("TLSMemoryAudit", folder=outerFolder):
