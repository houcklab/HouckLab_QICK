...
METHOD_LABELS = {
    ...
    "per_shot_25": "reset on, per-shot park, 25 us",
    "refresh_25": "reset on, pre-shot park refresh, 25 us",
    "refresh_100": "reset on, pre-shot park refresh, 100 us",
...
def _method_config(method):
    ...
    if method == "passive_1000":
        return "none", 1000.0
    if method in REFRESH_DELAYS_US:
        return "opx_unbounded", REFRESH_DELAYS_US[method]
    if method == "per_shot_25":
        return "opx_unbounded", 25.0
    if method == "per_shot_1000":
        return "opx_unbounded", 1000.0
    raise ValueError(f"unknown park-refresh T1 method {method!r}")


def _method_overrides(method):
    refresh = str(method) in REFRESH_DELAYS_US
    return {
        "opx_persistent_park": refresh,
        "opx_refresh_park_before_shot": refresh,
    }
...
def _final_evaluation(fits, round_fits):
    ...
    selected = next(
        (
            ...
            if row["method"] in REFRESH_DELAYS_US and row["passed"]
        ),
        None,
    )
...
def main():
    ...
    benchmark.ACTIVE_RELAX_US = min(REFRESH_DELAYS_US.values())
    benchmark.EXCURSION_GAIN = -20000.0
    benchmark.OUTPUT_TAG = "T1_flux_ramp_park_refresh_paired"
