...
GAIN_CURVE_FREQUENCY_HALF_SPAN_MHZ = 50.0
GAIN_CURVE_FREQUENCY_STEP_MHZ = 1.0
DELAY_US = (0.5, 2.0, 5.0, 10.0, 20.0, 40.0, 60.0, 80.0, 120.0, 160.0)
GAIN_CURVE_OFFSETS_DAC = (0, 500, 1000, 1500)
GAIN_CURVE_DELAY_US = 5.0
SHOTS = 100
INTERLEAVE_ROUNDS = 4
...
EDGE_GUARD_MHZ = 1.0
MIN_CURVE_FREQUENCY_EXCURSION_MHZ = 0.5
MIN_CURVE_R_SQUARED = 0.95
...
def main():
    ...
    curve_frequency_mhz = build_frequency_axis_mhz(
        center_mhz,
        GAIN_CURVE_FREQUENCY_HALF_SPAN_MHZ,
        GAIN_CURVE_FREQUENCY_STEP_MHZ,
    )
    now = datetime.now()
    output_dir = (
        Path(outerFolder)
    ...
    metadata = {
        ...
        "gain_curve_frequency_half_span_mhz": float(GAIN_CURVE_FREQUENCY_HALF_SPAN_MHZ),
        "gain_curve_frequency_step_mhz": float(GAIN_CURVE_FREQUENCY_STEP_MHZ),
        "delay_us": delay_us,
        "gain_curve_offsets_toward_zero_dac": list(GAIN_CURVE_OFFSETS_DAC),
        "gain_curve_delay_us": float(GAIN_CURVE_DELAY_US),
        "shots": int(SHOTS),
    ...
    def extract_maps(mean_iq, delays, probe_frequency_mhz):
        ...
        phase_trace = extract_trace_from_map(
            phase_rad,
            probe_frequency_mhz / 1e3,
            np.asarray(delays) * 1e3,
            center_mhz / 1e3,
            center_mhz / 1e3,
            float(np.ptp(probe_frequency_mhz) / 2.0) / 1e3,
            **options,
        )
    ...
    def acquire_arm(name, target_gain, delays, compensation=None, probe_frequency_mhz=None):
        delays = np.asarray(delays, dtype=float)
        probe_frequency_mhz = np.asarray(
            frequency_mhz if probe_frequency_mhz is None else probe_frequency_mhz,
            dtype=float,
        )
        cfg = build_park_probe_config(
            BaseConfig,
            park_gain=int(target_gain),
            frequency_axis_mhz=probe_frequency_mhz,
            shots=SHOTS,
        ...
        magnitude_db, phase_rad, magnitude_trace, phase_trace = extract_maps(
            mean_iq,
            delays,
            probe_frequency_mhz,
        )
        ...
        np.savez_compressed(
            ...
            phase_rad=phase_rad,
            frequency_mhz=probe_frequency_mhz,
            delay_us=delays,
        ...
        raw_rows = np.column_stack((
            ...
            np.broadcast_to(probe_frequency_mhz[:, None], magnitude_db.shape).ravel(),
            magnitude_db.ravel(),
            phase_rad.ravel(),
        ))
        np.savetxt(
        ...
        if len(delays) > 1:
            phase_summary = summarize_park_trace(
                ...
                sweep_min_mhz=float(probe_frequency_mhz[0]),
                sweep_max_mhz=float(probe_frequency_mhz[-1]),
        ...
        pcm0 = axes[0].pcolormesh(delays, probe_frequency_mhz, magnitude_db, shading="auto")
        ...
        pcm1 = axes[1].pcolormesh(delays, probe_frequency_mhz, phase_rad, shading="auto")
    ...
    print("stage=gain_frequency_curve", flush=True)
    gain_direction = int(-np.sign(park_gain))
    curve_gains = np.asarray([
        park_gain + gain_direction * int(offset)
        for offset in GAIN_CURVE_OFFSETS_DAC
    ], dtype=int)
    if np.any(np.abs(curve_gains) > max_abs_gain):
        raise ValueError("Gain-frequency calibration exceeds DAC range")
    curve_arms = [
        acquire_arm(
            f"curve_gain_{gain:+d}",
            int(gain),
            [GAIN_CURVE_DELAY_US],
            probe_frequency_mhz=curve_frequency_mhz,
        )
        for gain in curve_gains
    ]
    curve_phase_frequency = np.asarray([arm["phase_frequency_mhz"][0] for arm in curve_arms], dtype=float)
    curve_magnitude_frequency = np.asarray([arm["magnitude_frequency_mhz"][0] for arm in curve_arms], dtype=float)
    curve_rows = np.column_stack((curve_gains, curve_phase_frequency, curve_magnitude_frequency))
    np.savetxt(
        output_dir / "gain_frequency_curve.csv",
        curve_rows,
        delimiter=",",
        header="gain_dac,phase_frequency_mhz,magnitude_frequency_mhz",
        comments="",
    )
    frequency_curve = fit_sweet_spot_frequency_curve(
        curve_gains,
        curve_phase_frequency,
        park_gain=park_gain,
        min_frequency_excursion_mhz=MIN_CURVE_FREQUENCY_EXCURSION_MHZ,
        min_r_squared=MIN_CURVE_R_SQUARED,
    )
    (output_dir / "gain_frequency_curve.json").write_text(json.dumps(frequency_curve, cls=NpEncoder, indent=2, sort_keys=True) + "\n")
    ...
    response = frequency_trace_to_step_response_from_sweet_spot(
        delay_us=delay_us,
    ...
    correction_fit["method"] = "park_frequency_sweet_spot_deconvolution"
    correction_fit["metadata"] = {
        "qubit": QUBIT,
        "flux_channel": int(BaseConfig["ff_ch"]),
        "dc_offset": park_gain,
        "baseline_dc_offset": 0,
        "sweet_spot_curvature_mhz_per_dac_squared": frequency_curve["curvature_mhz_per_dac_squared"],
    ...
    summary = {
        "park_gain_dac": park_gain,
        "gain_frequency_curve": frequency_curve,
