...
def test_tls_step4_smoke_runner_selects_only_corrected_long_time_grid(monkeypatch):
    calls = []
    runner = SimpleNamespace(
        LIVE_PLOTS=True,
        SET_YOKO=True,
        ...
        USE_RESONATOR_LOOKUP=True,
        RESONATOR_FIT_PARAMS=[1.0],
        RESET_MODE="active",
        P1_RESONATOR={"run": True},
        P2_QUBIT_SPEC_FULL={"run": True},
        P3_STEP_RESPONSE={"run_fit": True, "run_correct": True},
        P4_LONG_TIME={"run": False},
        P5_SS_CAL={"run": True},
        P6_3PT_T1={"run": True},
        P6_FULL_T1={"run": True},
    )
    runner.main = lambda: calls.append({
        ...
        "resonator_fit": runner.RESONATOR_FIT_PARAMS,
        "reset_mode": runner.RESET_MODE,
        "step4": dict(runner.P4_LONG_TIME),
        "selected": [
            runner.P1_RESONATOR["run"],
            runner.P2_QUBIT_SPEC_FULL["run"],
            runner.P3_STEP_RESPONSE["run_fit"],
    ...
    monkeypatch.setitem(sys.modules, module_name, runner)
    monkeypatch.setattr(Runners, "TLSSpectroscopy", runner, raising=False)
    path = Path(__file__).parents[1] / "production_tls_step4_smoke_q3.py"
    ...
    assert calls == [{
        ...
        "selected": [False, False, False, False, True, False, False, False],
    }]
...
def test_p2_small_excursion_probe_centers_flux_sweep_on_base_config_park(monkeypatch):
    calls = []
    runner = SimpleNamespace(
        LIVE_PLOTS=False,
        SET_YOKO=True,
        RESET_MODE="active",
        BaseConfig={"ff_park_gain": -25790},
        BASELINE_DC_OFFSET=0,
        USE_RESONATOR_LOOKUP=True,
        RESONATOR_FIT_PARAMS=[1],
        P1_RESONATOR={"run": True},
    ...
    monkeypatch.setitem(sys.modules, module_name, runner)
    monkeypatch.setattr(Runners, "TLSSpectroscopy", runner, raising=False)
    path = Path(__file__).parents[1] / "p2_small_excursion_probe_q3.py"
