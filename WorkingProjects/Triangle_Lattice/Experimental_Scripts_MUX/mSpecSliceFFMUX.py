...
from WorkingProjects.Triangle_Lattice.Basic_Experiments_Programs.AveragerProgramFF import RAveragerProgramFF
...
class QubitSpecSliceFFProg(RAveragerProgramFF):
    def initialize(self):
        ...
        #                          length=self.us2cycles(cfg["length"]))
        print(self.FFPulse)
        print(cfg["mixer_freq"], cfg["res_freqs"], cfg["res_gains"], cfg["length"], self.cfg["adc_trig_offset"])
