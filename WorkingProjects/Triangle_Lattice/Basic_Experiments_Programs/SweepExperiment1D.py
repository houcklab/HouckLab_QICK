...
class SweepExperiment1D(ExperimentClass):
    ...
    def acquire(self, angle=None, threshold=None, progress=False, plotDisp=True, plotSave=True, figNum=1):
        ...
        if angle is not None: self.data['data']['angle'] = angle
        if threshold is not None: self.data['data']['threshold'] = threshold
        ...
        for j, x_pt in enumerate(X):
            ...
            for ro_index in range(len(readout_list)):
                if self.z_value == 'contrast' or self.z_value == 'IQ':
                    ...
                    theta = IQ_angle(I_array[ro_index][:j+1], Q_array[ro_index][:j+1])
                    rotated_i, _ = rotate(theta, I_array[ro_index][:j+1], Q_array[ro_index][:j+1])
