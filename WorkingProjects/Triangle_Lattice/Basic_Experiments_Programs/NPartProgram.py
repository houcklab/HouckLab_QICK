...
from WorkingProjects.Triangle_Lattice.Basic_Experiments_Programs.AveragerProgramFF import AveragerProgramFF
import scipy

class ThreePartProgramTwoFF(AveragerProgramFF):
    pass

class ThreePartProgramOneFF(AveragerProgramFF):
    def initialize(self):
        cfg = self.cfg
    ...
    def body(self):
        ...
        # 2: FFExpt
        self.FFPulses_direct(self.FFExpt, self.cfg["expt_cycles"], self.FFPulse, IQPulseArray= self.cfg["IDataArray"])
        self.sync_all(gen_t0=self.gen_t0)
        ...
        self.FFPulses_direct(-1 * self.FFExpt, self.cfg["expt_duration"], -self.FFExpt - 2*(self.FFReadouts - self.FFExpt), IQPulseArray = IQ_Array_Negative,
                            waveform_label='FF2')
