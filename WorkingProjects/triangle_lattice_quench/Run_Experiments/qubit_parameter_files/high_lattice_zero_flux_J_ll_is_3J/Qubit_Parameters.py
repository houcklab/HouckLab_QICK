...
Expt_FF = FF_gains([14146, 15404, 17993, 14988, -10553, 19410, 22889, 11842])
...
Readout_4Q = [7809, -919, 1362, 11132, 0, 791, 2432, 12067]
...
ramp_params = {
    ...
    '34': {'Ramp':{'Init_FF': Expt_FF + [q2_det,q2_det,0,-5000,q2_det,q2_det, -15000, q2_det],
                        'Expt_FF': Expt_FF + [q2_det,q2_det,0,0,q2_det,q2_det, -15000, q2_det]}},
...
bs_params = {
    '1234_correlations': {'BS': {'BS_FF': [ -4795,  -5138, -16164, -20574,  -7500,  22806,   3328,   -705]},
                          't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),

                          'ij_samples':[0, 1, 1, 0, 0, 0, 1, 0],
                          'ij_gains':[ 4703,  6381,  -7231, -2652, -8723, 20700, 5000,  5664],
                          'pad_bs': [1, 0, 0, 1, 0, 0, 0, 1],
                          'exact_t_bs': [84, 84, 87, 87, 60, 60, 65, 65]
                          },

    '1234_bonds': {'BS': {'BS_FF': [ -4795,  -5006, -16164, -20504,  -7459,  22806,   3328,   -705]},
                          't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),
                          'ij_samples':[2, 0, 2, 0, 0, 2, 0, 2],
                          'ij_gains': [11900, 15452, 8400, 15271, -9948, -14000, 24700, 22034],
                          'pad_bs' : [0, 2, 0, 2, 2, 0, 2, 0],
                          },

    '1245_correlations': {'BS': {'BS_FF': [-21105, -22074,   -618,  16550,  -7497, -28060, -25722, -24395]},
                                 't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),
                                 'ij_samples':[0, 1, 0, 0, 0, 0, 0, 0],
                                 'ij_gains':[-3398, 671,  8883, 15479, -8723, -4119,  -511, -6181],
                                 'pad_bs':   [1, 0, 0, 0, 0, 0, 0, 0],
                                 'exact_t_bs': [58,58,82,82,82,82,82,82]},


    '1267_correlations': {'BS': {'BS_FF':[-12843, -13620,  25392,  27560,      0, -16544, -13249,  24395]},
         't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),
                'ij_samples': [0, 1, 0, 0, 0, 1, 0, 0],
                'ij_gains': [  700,   3300, 21888, 21415, -4974,  -9000,  5725, 18214],
                'pad_bs':   [1, 0, 0, 0, 0, 0, 1, 0],
                'exact_t_bs': [67,67,67,67,67,67,67,67]
                          },


    '2345_correlations': {'BS': {'BS_FF': [-26873,  -5270,  -2519,  16868,  -7497, -11557,  -9187,  24395]},
                          't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),
                          'ij_samples':[0, 0, 1, 0, 0, 1, 0, 0],
                          'ij_gains':[-6299,  5157,  800, 15479, -8723,  -379,  7756, 18214],
                          'pad_bs' :   [0, 1, 0, 0, 0, 0, 1, 0],
                          'exact_t_bs': [64, 64, 64, 79, 79, 65, 65, 65],
                          },


    '2345_bonds': {'BS': {'BS_FF': [-26873,  -5270,  -2734,  16868,  -8410, -11613,  -9187,  24395]},
                          't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),
                          'ij_samples':        [0, 2, 0, 2, 0, 0, 2, 0],
                          'ij_gains': [14276, 16200, 18385, -7000, -9948, 19823, 10800, 12034],
                          'pad_bs' :  [2, 0, 2, 0, 2, 2, 0, 2],
                          # 'exact_t_bs': [65, 58, 58, 55, 55, 65, 65, 65],
                          },


    '2356_correlations': {'BS': {'BS_FF': [ 26873, -22300, -16339,  -3838,  -7497,  22667, -25722, -24395]},
                              't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),
                              'ij_samples':        [0, 0, 1, 0, 0, 0, 0, 0],
                              'ij_gains':[20574, -3311,  -6800,  5716, -8723, 20700,  -511, -6181],
                              'pad_bs':            [0, 1, 0, 0, 0, 0, 0, 0],
                             'exact_t_bs':[59, 59, 59, 73, 73, 73, 73, 73]
                              },


    '2378_correlations': {'BS': {'BS_FF': [ 26873,  -9436,  -6205,  27560,      0,  28060,  -8258, -11351]},
                          't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),
                            'ij_samples': [0, 0, 1, 0, 0, 0, 1, 0],
                            'ij_gains': [20574,  3084,  2000, 21415, -4974, 23941,  756,   341],
                            'pad_bs': [0, 1, 0, 0, 0, 0, 0, 1],
                            'exact_t_bs':[61, 61, 61, 61, 62, 62, 62, 62]
                        },


    '3467_correlations': {'BS': {'BS_FF': [ 26873,  29001, -16100, -20612,      0,  -946,    370,  24395]},
                         't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),
                          'ij_samples': [0, 0, 1, 0, 0, 1, 0, 0],
                          'ij_gains': [20574, 22226,  -9000, -2652, -4974,  16000, 12535, 18214],
                          'pad_bs':     [0, 0, 0, 1, 0, 0, 1, 0],
                          'exact_t_bs':[54, 54, 54, 54, 66, 66, 66, 66]
                          },



    '4578_correlations': {'BS':{'BS_FF': [-26873, -29001,  -1576,  16752,  -7497,  -2529, -14979, -18332]},
                          't_offset': np.array([16, 18,  6, 18,  5,  7,  7,  0]),
                          'ij_samples': [0, 0, 0, 0, 0, 0, 1, 0],
                          'ij_gains': [-6299, -6775,  8404, 15479, -8723,  8647,  -6000, -3149],
