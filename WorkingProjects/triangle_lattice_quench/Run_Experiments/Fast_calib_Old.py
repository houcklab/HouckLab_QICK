...
for i in [8]:
    ...
    for Q in [3,4,5,6,7,8]:
        ...
        Second_Spec_params = {"qubit_gain": 100, "SpecSpan": 15, "SpecNumPoints": 71,
                                'Gauss': False, "sigma": 0.05, "Gauss_gain": 1200,
                                'reps': 144}


        Amplitude_Rabi_params = {"max_gain": 10000, 'relax_delay':100,
                                 'expts':51} # overriding default of 31

        # Amplitude_Rabi_params = {"max_gain": 22000, 'relax_delay': 100}  # overriding default of 31

        # if Q in [3, 5,8 ]:
        #     Amplitude_Rabi_params['max_gain'] *= 1.5


        SS_R_params = {"Shots": 2*400, 'relax_delay':150,
                       "gain_start": 600, "gain_stop": 2400, "gain_pts": 7,
                       "span": 1, "trans_pts": 5, 'number_of_pulses': 1,
                       'qubit_sweep_index':OptReadout_index}



        SS_Q_params = {"Shots": 3*500, 'relax_delay':150,
    ...
    # don't use f for filename because f = False ahaha
    with open(f'{varname_FF}.txt', 'w') as filename:
        for qconf in Qubit_configs:
            print(qconf, file=filename)
