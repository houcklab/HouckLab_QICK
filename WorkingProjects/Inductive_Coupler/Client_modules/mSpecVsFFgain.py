...
class LoopbackProgramTrans(AveragerProgram):
    ...
    def initialize(self):
        ...
        ff_freq = self.freq2reg(cfg["ff_freq"], gen_ch=ffChannel)  # convert frequency to dac frequency (ensuring it is an available adc frequency)
...
class SpecVsFFgain(ExperimentClass):
    """
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, plotSave = True, figNum = 1):
        ...
        gainVec = np.linspace(expt_cfg["gainStart"],expt_cfg["gainStop"], expt_cfg["gainNumPoints"])
        ...
        Y = gainVec
        Y_step = Y[1] - Y[0]
        Z_trans = np.full((expt_cfg["gainNumPoints"], expt_cfg["TransNumPoints"]), np.nan)
        Z_spec = np.full((expt_cfg["gainNumPoints"], expt_cfg["SpecNumPoints"]), np.nan)

        ### create an initial data dictionary that will be filled with data as it is taken during sweeps
        self.trans_Imat = np.zeros((expt_cfg["gainNumPoints"], expt_cfg["TransNumPoints"]))
        self.trans_Qmat = np.zeros((expt_cfg["gainNumPoints"], expt_cfg["TransNumPoints"]))
        self.spec_Imat = np.zeros((expt_cfg["gainNumPoints"], expt_cfg["SpecNumPoints"]))
        self.spec_Qmat = np.zeros((expt_cfg["gainNumPoints"], expt_cfg["SpecNumPoints"]))
        self.data= {
            'config': self.cfg,
        ...
        for i in range(expt_cfg["gainNumPoints"]):
            ...
            self.cfg['ff_gain'] = int(gainVec[i])
            ...
            axs[0].set_ylabel("FastFlux Gain (a.u.)")
            ...
            axs[1].set_ylabel("FastFlux Gain (a.u.)")
