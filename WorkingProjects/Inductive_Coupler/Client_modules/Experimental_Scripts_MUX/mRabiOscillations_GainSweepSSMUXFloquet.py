...
class Oscillations_Gain_SSMUXFloquet(ExperimentClass):
    """
    Basic T1
    """
    ...
    def acquire(self, threshold = None, angle = None, progress=False, figNum = 1, plotDisp = True,
        ...
        Floquet_Phases = self.cfg['Floquet_Phases']
        ...
        times = np.arange(0, len(self.cfg["IDataArray"][3])) * 2.32515 / 16 * 1e-3
        for i in self.cfg['Floquet_Drive']:
            self.cfg["IDataArray"][i - 1] += coupler_pulse(times, Floquet_Gains[i - 1], Floquet_Frequencies[i - 1],
                                                           Floquet_Phases[i - 1])
            # print(coupler_pulse(times, Floquet_Gains[i], Floquet_Frequencies[i],
            #                                                Floquet_Phases[i])[:100])
            self.cfg["IDataArray"][i - 1][self.cfg["IDataArray"][i + 1] > 32768] = 32768
            self.cfg["IDataArray"][i - 1][self.cfg["IDataArray"][i + 1] < -32768] = -32768
        ...
        for i in range(self.cfg["gainNumPoints"]):
            ...
            if type(self.cfg["IDataArray"][0]) != type(None):
                self.cfg["IDataArray"][self.cfg["qubitIndex"] - 1] = Compensated_Pulse(self.cfg['FF_Qubits'][
                                                                                           str(self.cfg["qubitIndex"])]
                                                                                       ['Gain_Expt'],
                                                    self.cfg['FF_Qubits'][str(self.cfg["qubitIndex"])]['Gain_Pulse'],
                                                                                     self.cfg["qubitIndex"])

                self.cfg["IDataArray"][self.cfg["qubitIndex"] - 1] += coupler_pulse(times, gainVec[i],
                                                                                    Floquet_Frequencies[self.cfg["qubitIndex"] - 1],
                                                                                    Floquet_Phases[self.cfg["qubitIndex"] - 1])
                print(gainVec[i], Floquet_Frequencies[self.cfg["qubitIndex"] - 1], Floquet_Phases[self.cfg["qubitIndex"] - 1], self.cfg['FF_Qubits'][
                                                                                           str(self.cfg["qubitIndex"])]
                                                                                       ['Gain_Expt'], self.cfg['FF_Qubits'][
                                                                                           str(self.cfg["qubitIndex"])]
                                                                                       ['Gain_Pulse'])
                self.cfg["IDataArray"][self.cfg["qubitIndex"] - 1][
                    self.cfg["IDataArray"][self.cfg["qubitIndex"] - 1] > 32768] = 32768
                self.cfg["IDataArray"][self.cfg["qubitIndex"] - 1][
                    self.cfg["IDataArray"][self.cfg["qubitIndex"] - 1] < -32768] = -32768
            print('Printing')
            print(np.round(self.cfg["IDataArray"][0][:20], 4))
...
#
#     np.arange(0, len(blank)) * 2.32515 / 16 * 1e-3