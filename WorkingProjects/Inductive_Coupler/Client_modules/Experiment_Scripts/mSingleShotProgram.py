...
class LoopbackProgramSingleShot(RAveragerProgram):
    ...
    def initialize(self):
        ...
        for ro_ch in cfg["ro_chs"]:
            ...
            self.declare_readout(ch=ro_ch, freq=read_freq,
        ...
        self.set_pulse_registers(ch=cfg["res_ch"], style=self.cfg["read_pulse_style"], freq=read_freq, phase=0, gain=int(cfg["read_pulse_gain"]),
                                 length=self.us2cycles(self.cfg["readout_length"]) + self.us2cycles(self.cfg["adc_trig_offset"]),
                                 ) # mode="periodic")
