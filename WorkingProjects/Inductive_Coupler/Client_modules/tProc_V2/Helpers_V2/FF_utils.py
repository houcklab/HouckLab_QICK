...
def FFPulses(instance, list_of_gains, length_us, t_start='auto', IQPulseArray=None, waveform_label="FF"):
    for i, gain in enumerate(list_of_gains):
        ...
        if IQPulseArray[i] is None:
            instance.add_pulse(ch=instance.FFChannels[i], name='ff_pulse{}'.format(instance.FFChannels[i]), style='const', freq=0, phase=0,
                                         gain=gain,
                                         length=length)
        else:
    ...
    for channel in instance.FFChannels:
        ...
        # print(t_start_)
        instance.pulse(ch=channel, t=t_start_, name='ff_pulse{}'.format(channel))
