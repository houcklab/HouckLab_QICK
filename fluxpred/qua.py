"""Offline QUA DC schedule lowering, without OPX output filters or hardware imports.

This plan describes requested QUA timing. Actual set_dc_offset/wait scheduling
must still be checked with the installed QOP compiler/simulator. The instruction
estimate is a source-operation budget, not a compiled instruction-memory count.
"""
...
def compile_command(command, *, park_v, scale_v, clock_ns=4,
    ...
    """Lower a normalized absolute command to literal DC writes and waits.

    Voltage is ``park_v + scale_v * command.values``. Edges and voltages are
    rounded with numpy.rint (ties to even); cumulative edges prevent rounding
    drift. Every realized segment must be 4..2**31-1 clocks. No clipping or
    segment deletion is allowed. A scalar output_limit_v means the range
    [-limit, limit - voltage_quantum_v], including the initial park voltage.
    The command must already contain its complete outbound/return/park history.
    Its final level persists after the schedule; no extra transition is added.
    """
    ...
    instruction_estimate = 1 + 2 * values.size  # initial align; N DC writes + N waits
...
def reconstruct_command(plan):
    """Return the normalized command implied by the quantized backend plan."""
...
def _validate_flux_config(config, flux_element):
    ...
    if output.get("filter"):
        raise ValueError("software predistortion requires an empty OPX output filter on the flux port")


def emit(plan, flux_element, *, config, sync_elements, qua=None):
    """Emit after active reset and a verified settled-park initial state.

    Caller contract: all reset operations precede this call and the physical
    line is at the model's settled park state. This function aligns flux and
    sync_elements ONCE, then advances ONLY the flux timeline. Schedule drive or
    probe delays on their own elements immediately after this call. Do not
    align with flux again until those operations are scheduled, otherwise they
    would start after the complete flux waveform. Variable active-reset holds
    shift the common start; they are not permitted inside this prepared trace.

    The final voltage persists. Carry physical state through return and park
    with the shared software model; emitting an isolated reversed step or
    assigning a model state to zero does not remove analog memory.
    """
