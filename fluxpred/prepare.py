"""Prepare inspectable dedicated shot banks; never connect to hardware.

Modes: neutral (new state-space artifact), legacy-piecewise (existing step
command with explicit causal return), uncorrected. Existing legacy runners are
unchanged. --diagnostic is required for candidates failing the scientific gate.
Even a passing gate produces only plans: real compiler/timing checks remain.
"""
...
def legacy_round_trip(document, *, hold_ns, recovery_ns):
    ...
    # Validates every edge/level and keeps the final multiplier as its DC tail.
    ...
    # A zero gap before a later table edge is not a settled filter. Include the
    # entire delayed return response, even when the current coefficient is zero.
    ...
    if abs(value[-1]) > 1e-12:
        # The final cancelling jump can coincide exactly with the horizon.
        # Emit its terminal park explicitly so hardware cannot latch the
        # preceding interval forever.
...
def main():
    ...
    # Keep long numerical vectors on one line so metadata/timing can be read
    # without thousands of generated lines. The result remains ordinary JSON.
