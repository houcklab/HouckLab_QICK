"""Lower complete piecewise commands to QICK tProc-v1 constant pulses.

This module imports no QICK or hardware connection code. It does not design an
inverse, restart filter state, infer a return edge, or clip a command. Times are
generator fabric ticks, not DAC sample ticks or tProc ticks.
"""
...
MIN_CYCLES = 3
MAX_CYCLES = 65000
# Allow up to three instructions per immediate register write plus pulse/time
# setup. This intentionally exceeds the usual tProc-v1 const lowering. The
# installed QICK compiler and the complete surrounding program must still be
# compiled and counted before acquisition; this is only a planning allowance.
...
def compile_command(command, *, park_gain, scale_gain, clock_ns,
    ...
    """Quantize an entire causal command without changing its history.

    ``gain = park_gain + scale_gain * command.values``. ``scale_gain`` is a
    signed, finite, nonzero DAC excursion. Cumulative edges and gains use
    nearest-even rounding. Adjacent equal integer gains merge before the
    minimum-duration check. Any distinct segment shorter than three fabric
    ticks is rejected, including an edge that disappears under quantization.

    The instruction allowance includes ``reserved_instructions`` for caller
    code. It does not certify the actual compiled program's memory usage.
    """
    ...
    # Above 2**53 floating point cannot encode each cumulative tick exactly.
    ...
    for gain, total in merged:
        ...
        # Balanced splitting avoids a 1- or 2-cycle remainder at the end.
...
def reconstruct_command(plan):
    """Return the normalized command represented by the emitted DAC integers."""
...
def validate_program_compatibility(plan, prog, *, channel):
    """Check fabric clock and DAC limits before modifying program instructions."""
...
def emit(plan, prog, *, channel):
    """Queue the complete flux timeline; never synchronize other channels.

    Caller establishes the shared origin before calling, then queues qubit and
    readout operations on their own timelines. ``stdysel='last'`` retains the
    final command after the explicit horizon. A full return schedule must
    therefore include its final park level explicitly.
    """
