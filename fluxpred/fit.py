"""Regularized known-input identification and owned stable inverse fitting.

The fitted plant is P(s)=1+sum(a_j*s/(s+1/tau_j)). A trace offset absorbs
static calibration error; it is reported, never interpreted as dynamic gain.
This model assumes a settled initial park. Data lacking history cannot certify
that assumption. A frozen probe is approximated by a uniform voltage average,
not a full driven-qubit lineshape simulation.
"""
...
def highpass_features(command, time_ns, taus_ns, *, probe_ns=0, freeze_probe=True):
    ...
    if probe_ns and not freeze_probe:
        ...
        return u, features
    # A truncated hold stops before an equal-time scheduled update. The level
    # present just before the probe is then latched throughout its duration.
...
def fit_plant(traces, taus_ns, *, regularization=1e-4, max_l1=0.25):
    ...
    # Invoke strict pole/stability validation; DC remains exactly one.
    ...
    for trace in traces:
        ...
        # Equal total weight per trace, independent of sample count.
...
def fit_inverse(plants, taus_ns, *, regularization=1e-4, horizon_ns=500000, sample_ns=4000, max_amplitude=1.0):
    """Fit a bounded stable basis inverse to one or several identified plants.

    The emitted interval-mean waveform is in the design matrix, so its hold
    approximation is not hidden in a post-fit conversion. No free time bins.
    """
    tau = np.asarray(taus_ns, float)
...
def interpolate_plant(amplitudes, coefficients, amplitude):
    """LOAO diagnostic: constant boundary prediction, with out-of-hull flag.

    Boundary predictions do not establish extrapolation validity. Deployment
    still rejects desired amplitudes beyond the calibrated model domain.
    """
