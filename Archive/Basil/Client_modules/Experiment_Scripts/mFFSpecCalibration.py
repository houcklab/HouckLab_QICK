...
class LoopbackProgramTrans(AveragerProgram):
    ...
    def initialize(self):
        ...
        self.set_pulse_registers(ch=cfg["res_ch"], style="const", freq=read_freq, phase=0,
                                 gain=cfg["pulse_gain"],
    ...
    def FFPulses(self, list_of_gains, length_us):
...
class LoopbackProgramSpecSlice(RAveragerProgram):
    ...
    def initialize(self):
        ...
        self.delay = self.cfg["delay_time"]
        ...
        # Sara: add gen_ch specification for us2cycles conversion for length of readout pulse
        ...
        # Sara: self.res_wait doesn't seem to be used so I am commenting it out,
        # but just so you know, wait times use the tProc clock (different from gen_ch/ro_ch clocks) so
        # self.qubit_length isn't correct since it was previously defined to use the
        # gen_ch clock. instead you should have
        #   self.res_wait = self.us2cycles(cfg["sigma"] * 2) + self.us2cycles(self.delay)
        # Original (commented out):
        # self.res_wait = self.qubit_length + self.us2cycles(self.delay)
    ...
    def body(self):
        ...
        # Sara: Changed this FFPulses so that the us2cycles conversion is done inside the function
        # I tried to fix all uncommented instances of this below FFPulses and the optimized one
        self.FFPulses(self.FFRamp, 2)  # Sara: Length specified in us in FFPulses argument; initialize flux vals + 2 us

        # self.pulse(ch=self.cfg["qubit_ch"], t=self.us2cycles(self.delay + 2))  # play probe pulse
        self.pulse(ch=self.cfg["qubit_ch"], t=self.delay + self.us2cycles(2))  # probe pulse at 2 us + delay FIXME CLOCK
        self.FFPulses(self.FFRamp, 0.02,
        ...
        self.FFPulses_Optimized(self.FFExpts, 2 * self.cfg["sigma"] + self.cycles2us(self.delay)  # FIXME CLOCK
                                + 0.5, idata_pulse=True, IQPulseArray=self.cfg['IQPulseArray'])   # compensated step fn
        ...
        self.sync_all()  # reset t=0 to latest time
        self.FFPulses(self.FFReadouts, self.cfg["length"])
        self.measure(pulse_ch=self.cfg["res_ch"],
                     adcs=[0],
                     adc_trig_offset=self.us2cycles(self.cfg["adc_trig_offset"]),
        ...
        # self.FFPulses_Optimized(-1 * self.FFExpts, 2 * self.cfg["sigma"] + self.delay
        #                         + 0.5, idata_pulse=True,
        #                         waveform_label='FF2')  # , IQPulseArray = self.cfg['IQPulseArray'])
        self.FFPulses_Optimized(-1 * self.FFExpts, 2 * self.cfg["sigma"] + self.cycles2us(self.delay)  # FIXME CLOCK
                                + 0.5, idata_pulse=True,
                                waveform_label='FF2')  # , IQPulseArray = self.cfg['IQPulseArray'])  # net 0-flux step
        self.FFPulses(-1 * self.FFReadouts, self.cfg["length"])

        self.sync_all(self.us2cycles(5))

    def FFPulses(self, list_of_gains, length_us, t_start=None, idata_pulse=False, waveform_label='FF'):
        for i, gain in enumerate(list_of_gains):
    ...
    def FFPulses_Optimized(self, list_of_gains, length_us, t_start=None, idata_pulse=False,
                           IQPulseArray=[None, None, None], waveform_label="FF"):
        for i, gain in enumerate(list_of_gains):
            # Sara edited to correctly convert to cycles using the correct gen_ch
            length = self.us2cycles(length_us, gen_ch=self.FFChannels[i])
            # print(length)
    ...
    def FFDefinitions(self):
        ...
        # Finish FF
        self.FF_Channel1, self.FF_Gain1_read = self.cfg["FF_list_read"][0]
        self.FF_Channel2, self.FF_Gain2_read = self.cfg["FF_list_read"][1]
        self.FF_Channel3, self.FF_Gain3_read = self.cfg["FF_list_read"][2]
        ...
        self.FF_Gain1_init = self.cfg["FF_list_init"][0][1]
        self.FF_Gain2_init = self.cfg["FF_list_init"][1][1]
        self.FF_Gain3_init = self.cfg["FF_list_init"][2][1]

        self.FFChannels = np.array([self.FF_Channel1, self.FF_Channel2, self.FF_Channel3])
        self.FFReadouts = np.array([self.FF_Gain1_read, self.FF_Gain2_read, self.FF_Gain3_read])
        self.FFExpts = np.array([self.FF_Gain1_step, self.FF_Gain2_step, self.FF_Gain3_step])
        self.FFRamp = np.array([self.FF_Gain1_init, self.FF_Gain2_init, self.FF_Gain3_init])
    ...
    def acquire(self, plotDisp=True, plotSave=True, figNum=1, i_data=False, q_data=False):
        ...
        for i in tqdm(range(self.cfg["DelayPoints"]), position=0, leave=True):
            ...
            if i == 0:  # if first sweep add a colorbar
                ...
                cbar0 = fig.colorbar(ax_plot_0, ax=axs[0], extend='both')
                cbar0.set_label('a.u.', rotation=90)
            axs[0].set_ylabel("Delay Time (us)")
            ...
            axs[1].set_ylabel("Delay Time (us)")
            ...
            if i == 0:  # during the first run create a time estimate for the data aqcuisition
                ...
                timeEst = (t_delta) * self.cfg["DelayPoints"]  # estimate for full scan
