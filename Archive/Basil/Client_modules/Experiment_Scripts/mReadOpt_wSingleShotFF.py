...
from q3diamond.Client_modules.Helpers.MixedShots_analysis import *
from tqdm.notebook import tqdm
import time
import datetime
import q3diamond.Client_modules.PythonDrivers.YOKOGS200 as YOKOGS200

from q3diamond.Client_modules.mTransmissionFF import SingleToneSpectroscopyProgramFF
from q3diamond.Client_modules.mSpecSliceFF import PulseProbeSpectroscopyProgramFFMultipleFF
from q3diamond.Client_modules.Experiment_Scripts.mSingleShotProgramFF import LoopbackProgramSingleShot
from q3diamond.Client_modules.Experiment_Scripts.mSingleShotProgramFF_HigherLevels import * #SingleShotProgramFF_2States, hist



class ReadOpt_wSingleShotFF_HigherStates(ExperimentClass):
    """
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, plotSave = True, calibrate=False, cavityAtten=None,
                figNum = 1, ground_pulse = 0, excited_pulse = 1):
        #### function used to actually find the cavity parameters
        expt_cfg = {
            ### define the attenuator parameters
            "cav_Atten_Start": self.cfg["cav_Atten_Start"],
            "cav_Atten_Stop": self.cfg["cav_Atten_Stop"],
            "cav_Atten_Points": self.cfg["cav_Atten_Points"],
        ...
        self.atten_fpts = np.linspace(expt_cfg["cav_Atten_Start"], expt_cfg["cav_Atten_Stop"], expt_cfg["cav_Atten_Points"])
        ...
        self.data= {
            ...
            'data': {'fid_mat': Z_fid, 'overlap_mat': Z_overlap,
                     'trans_fpts':self.trans_fpts, 'atten_fpts':self.atten_fpts,
                     }
        }
        ...
        for idx_atten in range(len(self.atten_fpts)):
            # start_2 = time.time()
            ### set the cavity attenuation
            cavityAtten.SetAttenuation(self.atten_fpts[idx_atten])

            ### start the loop over transmission points
            for idx_trans in range(len(self.trans_fpts)):
                self.cfg["read_pulse_freq"] = self.trans_fpts[idx_trans]
                i_g, q_g, i_e, q_e = self._acquireSingleShotData(ground_pulse=ground_pulse, excited_pulse=excited_pulse)
                ...
                Z_fid[idx_atten, idx_trans] = fid
                self.data['data']['fid_mat'][idx_atten, idx_trans] = fid

                #### plotting
                if idx_atten == 0 and idx_trans ==0:
                ...
                if self.cfg["qubit_pulse_style"] in ["flat_top", "arb"]:
                    ...
                    Z_overlap[idx_atten, idx_trans] = overlapErr
                    self.data['data']['overlap_mat'][idx_atten, idx_trans] = overlapErr
                ...
                if idx_atten == 0 and idx_trans ==0:  ### during the first run create a time estimate for the data aqcuisition
                    t_delta = time.time() - start  ### time for single full row in seconds
                    timeEst = t_delta * expt_cfg["cav_Atten_Points"] * expt_cfg["TransNumPoints"]  ### estimate for full scan
    ...
    def _acquireSingleShotData(self, ground_pulse = 0, excited_pulse = 1):
        ...
        if ground_pulse == 0:
            ...
            self.cfg["pulse_expt"]["pulse_12"] = False
        prog = SingleShotProgram(soccfg=self.soccfg, cfg=self.cfg)
        avgi, avgq = prog.acquire(self.soc, load_pulses=True)
        i_0, q_0 = prog.collect_shots()
        ...
        fid, threshold, angle = hist(data=data, plot=False, span=False, verbose=False)
        ...
        #
        # ### use the helper histogram to find the fidelity and such
        # fid, threshold, angle = hist_process(data=[i_0, q_0, i_1, q_1], plot=False, ran=None)
