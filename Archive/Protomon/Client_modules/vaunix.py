...
print('Device 0 Serial number:', str(ser_num))
...
# GetSerialNumber will return the devices serial number
ser_num = vnx.fnLDA_GetSerialNumber(Devices[1])
print('Device 1 Serial number:', str(ser_num))

# InitDevice wil prepare the device for operation
init_dev = vnx.fnLDA_InitDevice(Devices[1])
print('InitDevice returned', str(init_dev))
...
print('Set attenuation level for channel 1 of Device 0', end='')
atten = float(input(': '))
...
#### Device 1 ####
# GetNumChannels will return the number of channels on the device
num_channels = vnx.fnLDA_GetNumChannels(Devices[1]);
if num_channels < 1:
    num_channels = 1

# Input desired attenuation level for channel 1
print('Set attenuation level for channel 1 of Device 1', end='')
atten = float(input(': '))
attenuation = atten / .05
atten = round(attenuation)

# Select channel 1
channel_1 = vnx.fnLDA_SetChannel(Devices[1], 1)
if channel_1 != 0:
    print('SetChannel returned error', channel_1)

# Set attenuation level for channe 1
result_1 = vnx.fnLDA_SetAttenuationHR(Devices[1], int(atten))
if result_1 != 0:
    print('SetAttenuationHR returned error', result_1)

# Get channel 1 attenuation
result_1 = vnx.fnLDA_GetAttenuationHR(Devices[1])

# Use a for loop to loop through the remainder of the channels
for i in range(2, num_channels + 1):

    # Select the channel
    channel = vnx.fnLDA_SetChannel(Devices[1], i)
    if channel != 0:
        print('SetChannel returned an error', str(channel))
    ...
    # Set the attenuation for channel i
    result = vnx.fnLDA_SetAttenuationHR(Devices[1], int(atten))
    if result != 0:
        print('SetAttenuationHR returned error', result)
...
for i in range(2, num_channels + 1):
    if i > 0:
        ...
        # Get channel i's attenuation
        result = vnx.fnLDA_GetAttenuationHR(Devices[1])
...
# Always close the device when done with it
result = vnx.fnLDA_CloseDevice(Devices[1])
