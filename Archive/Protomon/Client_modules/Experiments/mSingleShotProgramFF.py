...
class LoopbackProgramSingleShotFF(RAveragerProgram):
    ...
    def initialize(self):
        ...
        ##### this was the original configuration that didnt work for some reason
        cfg["start"]=0
        ...
        self.cfg["ff_length_total"] = self.us2cycles(self.cfg["ff_length"]*12)
    ...
    def collect_shots(self):
        shots_i0=self.di_buf[0].reshape((self.cfg["expts"],self.cfg["reps"]))/self.cfg['read_length']
        shots_q0=self.dq_buf[0].reshape((self.cfg["expts"],self.cfg["reps"]))/self.cfg['read_length']
...
class SingleShotProgramFF(ExperimentClass):
    """
    Basic SingleShot experiement that uses fast flux pulses to set the flux point
    """
    ...
    def display(self, data=None, plotDisp = False, figNum = 1, ran=None, **kwargs):
        ...
        fid, threshold, angle = hist_process(data=[i_g, q_g, i_e, q_e], plot=plotDisp, ran=ran)
