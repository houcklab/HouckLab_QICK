...
from Protomon.Client_modules.Experiments.mSingleShotProgram import LoopbackProgramSingleShot
from Protomon.Client_modules.Experiments.mSingleShotPS import LoopbackProgramSingleShotPS
from Protomon.Client_modules.Experiments.mSingleShotPS_FF import LoopbackProgramSingleShotPS_FF
from Protomon.Client_modules.Experiments.mSingleShotProgramFF import LoopbackProgramSingleShotFF
...
class GainReadOpt_wSingleShot(ExperimentClass):
    """
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, plotSave = True, calibrate=False, cavitygain=None, figNum = 1,
                FF = False, PS = False, FF_PS = False):
        #### function used to actually find the cavity parameters
        expt_cfg = {
            ##### define gain parameters
            "trans_gain_start": self.cfg["trans_gain_start_readopt"],
            "trans_gain_stop": self.cfg["trans_gain_stop_readopt"],
            "trans_gain_num": self.cfg["trans_gain_num_readopt"],
            ### transmission parameters
            "trans_freq_start": self.cfg["trans_freq_start_readopt"],  # [MHz] actual frequency is this number + "cavity_LO"
            "trans_freq_stop": self.cfg["trans_freq_stop_readopt"],  # [MHz] actual frequency is this number + "cavity_LO"
            "TransNumPoints": self.cfg["trans_freq_num_readopt"],  ### number of points in the transmission frequecny
        }
        self.gain_fpts = np.linspace(expt_cfg["trans_gain_start"], expt_cfg["trans_gain_stop"], expt_cfg["trans_gain_num"], dtype=int)
        ...
        self.data= {
            ...
            'data': {'fid_mat': Z_fid, 'overlap_mat': Z_overlap,
                     'trans_fpts':self.trans_fpts, 'gain_fpts':self.gain_fpts,
                     }
        }
        ...
        for idx_gain in range(len(self.gain_fpts)):
            ...
            for idx_trans in range(len(self.trans_fpts)):
                ...
                Z_fid[idx_gain, idx_trans] = fid
                self.data['data']['fid_mat'][idx_gain, idx_trans] = fid
                ...
                if idx_gain == 0 and idx_trans == 0:  #### if first sweep add a colorbar
                ...
                if self.cfg["qubit_pulse_style"] in ["flat_top", "arb"]:
                    ...
                    Z_overlap[idx_gain, idx_trans] = overlapErr
                    self.data['data']['overlap_mat'][idx_gain, idx_trans] = overlapErr
                ...
                if idx_gain == 0 and idx_trans ==0:  ### during the first run create a time estimate for the data aqcuisition
                    t_delta = time.time() - start  ### time for single full row in seconds
                    timeEst = t_delta * expt_cfg["trans_gain_num"] * expt_cfg["TransNumPoints"]  ### estimate for full scan
