...
class PiOpt_wSingleShot(ExperimentClass):
    ...
    def acquire(self, progress=False, debug=False, plotDisp = True, plotSave = True, figNum = 1,
        ...
        expt_cfg = {
            ...
            "qubit_freq_start": self.cfg["qubit_freq_start_readopt"],
            "qubit_freq_stop": self.cfg["qubit_freq_stop_readopt"],
            "qubit_freq_num": self.cfg["qubit_freq_num_readopt"],  ### number of points
            ### define the qubit gain points
            "qubit_gain_start": self.cfg["qubit_gain_start_readopt"],
            "qubit_gain_stop": self.cfg["qubit_gain_stop_readopt"],
            "qubit_gain_num": self.cfg["qubit_gain_num_readopt"],  ### number of points
        }
            ...
            for idx_freq in range(len(self.freq_fpts)):
                ...
                if self.cfg["qubit_pulse_style"] in ["flat_top", "arb"]:
                    ...
                    while plt.fignum_exists(num=5):
                        plt.close(5)
                    mixed.PlotGaussFit(iname=self.iname,ind1=idx_gain, ind2=idx_freq)
