...
class LoopbackProgramSingleShotPS_FF(RAveragerProgram):
    ...
    def initialize(self):
        ...
        self.res_wait = self.qubit_pulseLength + self.us2cycles(0.02)
    ...
    def body(self):
        ...
        self.synci(self.us2cycles(self.cfg["read_length"]*2))
        #### forcibly reset the adc_ts and dac_ts since a sync_all cant be run
...
class SingleShotPS_FF(ExperimentClass):
    """
    ...
    def display(self, data=None, plotDisp = False, figNum = 1, ran=None, **kwargs):
        ...
        fid, threshold, angle = hist_process(data=[i_g, q_g, i_e, q_e], plot=plotDisp, ran=ran, figNum=figNum)
        plt.suptitle('selected data')
        plt.savefig(self.iname)

        # fid, threshold, angle = hist_process(data=[ig_1, qg_1, ie_1, qe_1], plot=plotDisp, ran=ran, figNum=figNum+1)
        # plt.suptitle('raw data 1')
        #
        # fid, threshold, angle = hist_process(data=[ig_0, qg_0, ie_0, qe_0], plot=plotDisp, ran=ran, figNum=figNum+2)
        # plt.suptitle('raw data 0')
