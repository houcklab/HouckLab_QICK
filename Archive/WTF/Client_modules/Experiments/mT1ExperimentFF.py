...
class LoopbackProgramT1ExperimentFF(RAveragerProgram):
    ...
    def initialize(self):
        ...
        self.maxWait = self.cfg["step"]*self.cfg["expts"]- self.cfg["start"]
    ...
    def body(self):
        ...
        self.pulse(ch=self.cfg["ff_ch"])  # play pulse
        self.pulse(ch=self.cfg["ff_ch"])  # play pulse
        #### play 10 more times to accound for the T1 waiting times
        self.pulse(ch=self.cfg["ff_ch"])  # play pulse
        self.pulse(ch=self.cfg["ff_ch"])  # play pulse
        self.pulse(ch=self.cfg["ff_ch"])  # play pulse
        self.pulse(ch=self.cfg["ff_ch"])  # play pulse
...
class T1ExperimentFF(ExperimentClass):
    """
    Basic T1 experiment with fast flux pulse
    """
    ...
    def acquire(self, progress=False, debug=False):
        ...
        prog = LoopbackProgramT1ExperimentFF(self.soccfg, self.cfg)
        ...
        ##### reset the mag to i or q for certain scans
        mag = avgi[0][0]
        ...
        #### define T1 function
        def _expFit(x, a, T1, b):
            return a * np.exp(-1 * x / T1) + b

        if mag[0] > mag[-1]:
            a_geuss = (np.max(mag)-np.min(mag))
            b_geuss = np.min(mag)
        else:
    ...
    def display(self, data=None, plotDisp = False, figNum = 1, **kwargs):
        ...
        plt.plot(times, avgi[0][0], 'o-', label="i")
        plt.plot(times, avgq[0][0], 'o-', label="q")
