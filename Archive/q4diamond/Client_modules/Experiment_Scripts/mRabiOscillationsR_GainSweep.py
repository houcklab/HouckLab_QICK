...
class OscillationsRProgram(RAveragerProgram):
    def initialize(self):
        cfg = self.cfg
        ...
        # load waveforms
        print("setting FFlength to 16")
        self.cfg['FFlength'] = 16
        FF.LoadWaveforms(self, prev_value=self.FFPulse)

        for rp, r_addr, res_r_mode, channel in zip(self.rps, self.r_addrs, self.res_r_modes, self.FFChannels):
            ...
            # set pulse registers to constant pulse with dummy gain of 0, to get mode code
            self.set_pulse_registers(ch=channel, freq=0, phase=0, gain=0, style='const', length=self.cfg['start'])
            self.mathi(rp, 4, res_r_mode, '+', 0)
            # self.regwi(rp, 5, self.cfg['start'])  # for wait time
            self.regwi(rp, 5, 0)  # for wait time
    ...
    def body(self):
        ...
        for FFChannel, rp, r_addr, res_r_mode, gain in zip(self.FFChannels, self.rps, self.r_addrs, self.res_r_modes,
                                                           self.FFExpts):
            ...
            self.set_pulse_registers(ch=FFChannel, freq=0, phase=0, gain=gain, style='const', length=self.cfg['start'])
            self.mathi(rp, res_r_mode, 4, '+', 0)  # restore value of res_r_mode for const pulse
            self.pulse(ch=FFChannel)

        print("2", self.dac_ts, self.adc_ts)
        self.sync(rp, 5)  # align channels and wait the extra length set by the register
    ...
    def update(self):
        ...
        # self.memwi(self.rps[0], 2, 124)
        # self.memwi(self.rps[0], 3, 125)
        # self.memwi(self.rps[0], 4, 126)
        # self.memwi(self.rps[0], 5, 127)
        ...
        for i, rp in enumerate(set(self.rps)):  # increment once per page! some channels use the same page
            ...
            self.mathi(rp, 1, 1, '+', 1)  # FIXME: assumes step == 1
            self.mathi(rp, 3, 3, '+', (self.cfg['FFlength'] // 16 + 2))  # address counter
            ...
            self.mathi(rp, 4, 4, '+', 1)  # increase length (mode) by 1 clock cycle
            self.mathi(rp, 5, 5, '+', 1)  # increase length (wait) by 1 clock cycle
...
class OscillationsR_Gain(ExperimentClass):
    """
    Two-qubit oscillations, varying relative qubit detuning
    """
    ...
    def acquire(self, threshold = None, angle = None, progress=False, debug=False, figNum = 1, plotDisp = True,
        ...
        if np.array(self.cfg["IDataArray"]).any() != None:
            ...
            self.cfg['FFlength'] = len(self.cfg["IDataArray"][0])

        start = time.time()
        for i in range(self.cfg["gainNumPoints"]):
            # start by resetting rfsoc?
            self.soc.reset_gens()
            ...
            if type(self.cfg["IDataArray"][0]) != type(None):
                self.cfg["IDataArray"][self.cfg["qubitIndex"] - 1] = \
                    Compensated_Pulse(int(gainVec[i]),
                                      self.cfg['FF_Qubits'][str(self.cfg["qubitIndex"])]['Gain_Pulse'],
                                      self.cfg["qubitIndex"])

            start = time.time()
            prog = OscillationsRProgram(self.soccfg, self.cfg)
            ...
            # print("addr:   ", self.soc.tproc.single_read(addr=125))
            # print("mode:   ", self.soc.tproc.single_read(addr=126))
            # print("wait:   ", self.soc.tproc.single_read(addr=127))
            # print("")
            ...
            self.data['data']["oscillation_I"][i, :] = avgi[0][0]
            self.data['data']["oscillation_Q"][i, :] = avgq[0][0]
            if np.abs(self.I_Range) > np.abs(self.Q_Range):
                Z_values[i, :] = (avgi[0][0] - self.I_Ground) / self.I_Range
            else:
                Z_values[i, :] = (avgq[0][0] - self.Q_Ground) / self.Q_Range
