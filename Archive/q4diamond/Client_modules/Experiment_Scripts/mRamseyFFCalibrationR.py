...
class RamseyFFCalRProg(RAveragerProgram):
    def initialize(self):
        ...
        f_res = self.freq2reg(self.cfg["pulse_freq"], gen_ch=self.cfg["res_ch"],
                              ro_ch=self.cfg["ro_chs"][0])  # convert f_res to dac register value
        self.set_pulse_registers(ch=self.cfg["res_ch"], style="const", freq=f_res, phase=self.cfg["res_phase"],
                                 gain=self.cfg["pulse_gain"], length=self.us2cycles(self.cfg["length"]))
        for ro_ch in [0, 1]:  # configure the readout lengths and downconversion frequencies
            self.declare_readout(ch=ro_ch, length=self.us2cycles(self.cfg["readout_length"]),
                                 freq=self.cfg["pulse_freq"], gen_ch=self.cfg["res_ch"])
