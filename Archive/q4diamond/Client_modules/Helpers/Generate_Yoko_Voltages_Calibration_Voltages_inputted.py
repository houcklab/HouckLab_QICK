...
offset_vector[0] += 0.0034
offset_vector[1] += 0.0147
offset_vector[2] += 0.0072
offset_vector[3] += -0.0147


# file_location = 'Z:\Jeronimo\Measurements\Qubit_Parameters'
yoko_voltages = pickle.load(open('Voltages.p', 'rb'))
yoko_voltages = yoko_voltages.T
print(yoko_voltages[:4])
...
qfs = []
cfs = []
for v in yoko_voltages:
    f4_q = Voltage_to_Freq(v, list_of_dictionaries, voltage_matrix, offset_vector)
    f4_c = [cavity_frequency_qubit(f4_q[i], 'qubit' + str(i + 1), cavity_dictionaries) * 1e3 for i in range(len(v))]
    vs_in.append(v)
    qfs.append(f4_q)
    cfs.append(f4_c)
...
pickle.dump(dat, open('Yoko_Voltages_0F_100_Dist.p', 'wb'))