...
offset_vector[0] += 0.0009
offset_vector[1] += -0.0008
offset_vector[2] += -0.001
offset_vector[3] += -0.0005

print(cavity_dictionaries)
maximum_total_voltage = 1.2
...
Q3_ranges = [-1.1, 0.4]
Q4_ranges = [-0.6, 1.05]
Qubit_ranges = [Q1_ranges, Q2_ranges, Q3_ranges, Q4_ranges]

points_per_qubit = 40
vs_in = []
qfs = []
cfs = []
for m, Q_range in enumerate(Qubit_ranges):
    npts = np.linspace(Q_range[0], Q_range[1], points_per_qubit)
    for n in npts:
        leftover = maximum_total_voltage - np.abs(n)
        if leftover <= 0:
            print('Voltage larger than max!!')
            break
        v4_in = (leftover * 1.5) * (np.random.rand(3) - 0.5)
        v4_in = np.insert(v4_in, m, n)
        for index_call in range(100):
            if sum(abs(v4_in)) < maximum_total_voltage:
                break
            v4_in = (leftover * 1.5) * (np.random.rand(3) - 0.5)
            v4_in = np.insert(v4_in, m, n)
        f4_q = Voltage_to_Freq(v4_in, list_of_dictionaries, voltage_matrix, offset_vector)
        for index_call in range(100):
            if qfs_conflict(f4_q):
                break
            v4_in = (leftover * 1.5) * (np.random.rand(3) - 0.5)
            v4_in = np.insert(v4_in, m, n)
            for index_call2 in range(100):
                if sum(abs(v4_in)) < maximum_total_voltage:
                    break
                v4_in = (leftover * 1.5) * (np.random.rand(3) - 0.5)
                v4_in = np.insert(v4_in, m, n)
...
voltages = list(np.linspace(-1, 1, 161))
freqs = []
for v in voltages:
...
pickle.dump(dat, open('Yoko_Voltages_0FAB_160.p', 'wb'))